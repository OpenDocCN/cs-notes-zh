# UIUC《高级数据结构｜CS 598 JGE Advanced Data Structures — Spring 2025》中英字幕 p09 -14-Feb 18： ST-trees.zh_en -BV1kWFGzsEmN_p9-

![](img/90273a93c4dc96860fc94e50fcdb48c5_0.png)

嗯。I'm going to try sharing my iPad wirelessly in the hopes that that will cut down in the number of times I have to reconnect the cable。

Because the video got cut off。Okay， so reminder。Um。Paper chase。Is due in two weeks。

 I think technically。The official deadline is two weeks from yesterday。But I'm not going to be。

 it's kind of a softish deadline so。tryry to get it in within two weeks if you can。

 but a few days of slack is not going to make that much difference。嗯。I am traveling next week。

So there that means。嗯。No class either Tuesday or Thursday and no office hours next Friday。
...
Somewhere between two and three hours。To review a paper when I know this topic of the paper really。
 ...
 really well。And so this is for me doing this assignment would take。You know， a couple hours a page。

Um， so give yourself plenty of time to do it and in particular start this week。

 don't wait until the last minute。All right。Any questions about？

TheThe assignment the link on the course webpage is now correct， apologize for screwing that up。
...
嗯。So everything should be ready to go。And I know at least a a couple of people have already actually started talking to me about things to look at。
...
Three types of operations。Cut link。This is historically called fine root。
 ...
 connecting two trees into one find root， you give it a node and say which tree am I in？There are。Um。

Subre operations， which could be things like， you know， find the sum， find the min， u。
...
And so I want to specify a subtree， what I mean here by a subtre is when I cut an edge。
 ...
 maybe the values in the。In the subreer booles， and I want the and of all of them or the exclusives or of all of them。
...
And there are variants where sometimes you want the weights on the nodes。

 sometimes you want the weights on the edges。Sometimes you want to think of the edges as being undirected。

Mostly the way I'm going to describe it， sometimes you want to think of the graph as being directed and then edges between the same pair of nodes in opposite directions might have。
...
Subre operations and structural operations。In log in amortize time。Okay， you。Imagine。

Doing an oiler tour of the tree， stick your hand out and touch the tree with your right hand。
...
And then links and cuts turn into splits and concatenations in these various Euler tours and subre operations turn into interval operations。

And we started talking about。嗯。ST trees， which are slater and T's original quote unquote。
...
U。That that we're going to assign every tree in my forest， a root。

 and I'll talk about in a minute how you would change the route。嗯。But for convenience。

 I just want to imagine that every tree has a root so I can talk about children and parents。A。And so。

Every node。Has。A。Preferred。Child。Pointer。Which could be null。

Or it could point to one of the notes to children。Okay， and so every node the this。Points。Toward。

The last node。啊。Accessed。In the the。Subree。Read it at thee。对。嗯。
...
Okay， so I start。Just to give a very。Small example of here。U。If I， for example。
...
Access the。All edges。On。path。From the root。To v。Are preferred。And no edges。Touching that path。Okay。

So this is the sort of high level structure that an ST tree maintains。
...
In a path tree。So this is a binary search tree of the path。Nodes。In depth order。Okay。
 ...
 so if you do an in order traersal of the binary tree。
...
And that goes from the root of the path tree。To the node representing the parent。
...
But that's not the only option it just makes the analysis simpler。系。So when I need to。Make。An edge。

Preferred。um。So this is an edge in the tree that I'm representing。
...
Concatenate two Pa trees。Okay， split and canca because I'm using sp trees。
...
Down to that node is preferred and all edges off that path is not preferred。So very crudely the time。

for。Access。A V。Is equal to。Let's see the number of non。Preferred edges。On the path。

2 v times big O login。And that's the number of non preferred edges before I did the access because after I do the access。
...
And then I splay that up to the root， and then I find a path parent pointer。
 ...
 I connect these two spplay trees， and then I continue displayplaying upwards。

 and then I connect two spplay trees and I continue displaying upwards。So in the end。嗯。
...
The number of non preferred edges on the path is the thing that determines。At a very broad scale。
 ...
 so before I go on from here， I need to make sure that people have questions before I refine the analysis。

 so eventually this is going to collapse down to just log in。Through amortization tricks。Okay。So。

The first thing。The first trick that I want to。啊。The first trick that I want to do is argue that it's actually not the number of preferred child pointers times big O of log n。
...
Is at most one plus。The new rank。Of the sorry minus the old rank。AV where rank。

Is defined to be the log of the size。And size is defined to be the sum over all descendants of the weight associated with W。

 now this weight is not something that's actually stored in the data structure。
...
Different assignments of weight are going to lead to different definitions of size and therefore different definitions of rank。
 ...
 and therefore by the axis limitma different amortized time bounds。
...
So this is one of the things that is sometimes difficult to remember about amortized analysis。
 ...
 different ways of distributing the work among different operations in time lead to different amortized time bounds。
...
ok。So I'm going to define。The the weight of W。To be。The number of descendants。Of W in the ST tree。
...
Sort of the number of nodes。In all。啊。Poth trees。connectedect。To W by a。Pa。Parent。Pointer。
...
So I'm just counting V itself。The weight of you， on the other hand， is five。

One for you itself and then four for that green path tree that's pointing into you and if there were other things save there were another path tree with its PowerPoter pointing into Z。

 that size of that other path tree would contribute to size of U。Or the weight of view。Okay。
...
Yes。Including itself。Yeah， including itself。Ples play trees in。

there any from weight arbitrarily hammerOkay， so this is a good question。
...
So it's really the log of the ratio between the old size and the new size and the old size。

So if you scale everything， that ratio doesn't change。So there's no sort of global way to。

 oh just make of the way it's all really small， that in the end doesn't have any effect。It or rather。

 it has the same effect on both the old and new ranking so the effect cancels out。嗯。Sorry。
...
Because otherwise you might define the rank as log of the size might be zero and then the rank's not really well defined。
 ...
 which is why I'm including the node itself in its own size calculation。对。Well， if I do that， then。

Let's think about what happens in。Okay， so here is。Some。Sorry。Here is some。Pass。That I want to。

Do an access on。And I want to access this node here。Okay so I've got。One， two， three。

 four preferred pads。On way that I need to traverse on the way to my target node V。
...
And if there's some other edge hanging off here that used to be preferred。
 ...
 then those are no longer preferred。Okay， so I potentially need in this instance， I need to do。F。

Spplay tree splits。And three s tree joints。Now in the ST tree。Okay， there's some splay tree leading。
...
Pointer to another。Noode。And go down here to V。啊。And。Okay， so。Now U V， W， and let's call the X。Okay。

So in the ST tree is I walk up from X。I follow a path from X to the root of its。Path tree。

 then cross a parent path pointer to a noW， then walk up to the root of Viitz Pa tree and so on until I get all the way up to the root of the Hu S tree。

Now。Everything's really is going to be dominated by the cost of displaying X。

Connecting it to W as a new child， then spplaying X again。

And then connecting it to V as a new child and displaying X again。

And then connecting it to you and new child and thening X again。So。What I'm going to get is。These。

Spplay operations。But now here， this is going to cost me one plus， let's。

 I'm going to say rank zero of x minus3 rank one of x。

 the subscript is how many playsplays have I already done so far yeah？要不0哎呀。Sorry， so。In the end。
...
the sizes don't change。嗯。And so instead of saying， well， yeah， each of these。

For bounds is at most log n？But in fact。They， you know， most of the stuff cancels out。

And so what I'm left with is just。This bunch of one pluses right， so this is the the number of。

Path three that I need to diverse。And then there's three times the new rank of x， well。

 at the end of the operation， x is the root。So its rank is。嗯。Just the size of the whole。Its upree。

And well， this negative part I can remove。So I end up with just this。ok。

Now I'm wearing big O glasses when I'm doing this， so all of those things when I say it's dominated by that I mean up to constant factors。

 so I really should be putting a big O around here。

But this is what I claimed earlier that the crude analysis is for every pathway through that I need to traverse。
 ...
 which means I actually only get one log in the end。Right。Okay。😊，Yeahや。The brand the final rank that。

It's can people walk。Okay， so the weight。Of a node is the number of nodes in the whole structure that are ultimately connected by a path parent pointer。

 the size of the top。Pastry。Is the sum of all of those weights？

Which means it's the number of nodes in the whole cluster。The whole collection of Pares。

Because every path tree is going to be connected to some node in this top。

Path tree eventually or the last step is a path PowerPoer。So the size of x。

 when x is the root of the ST tree is exactly n。So the rank is exactly log in now that's it。Okay。

 great。U。So。The second trick。Is that。Is to show that the amortized。Number of path trees。

That I need to diverse is only constant。Now， it's possible in the worst case that doing a single access。

We'll have to actually touch。A linear number of past trees。 So， for example。
...
Then I'm going to have to do a linear number of joint operations to connect all these things up into to sew them all up into a single path tree。

 so in the worst case， I absolutely can't say this。

 but again I'm not really worried about the behavior。Of one operation。

 I'm really worried about the overall running time of in particular a long sequence。

Of these operations。So。All right。So the way that I'm going to do this。嗯。So I'm going to say， okay。

The number of。Preferred。Poiner changes。This is the thing that I want to analyze and ultimately what I want to do is analyze the total of this quantity summed over all operations that I need to perform。

系。嗯。Okay， so。诶。Forgiven。Yeah， so。I need to， you， so the claim。Is that the amortized。

Number of preferred child planers is changes is order one。Um。So this analysis relies。On。

Something called a heavy light decomposition。Now。What I said at the beginning when I started talking about dynamic forest and saying the language is confusing because I have to be very careful to distinguish between the tree that I'm representing。
...
Into paths。It's not the one that's used to define preferred pads。 It's something else， Okay。

 so I'm going to say that an edge。From。It's a parent you to child V in this represented tree。

Is heavy。If。The size of the subre。Rooted at V is at is actually strictly greater than half。

The size of the subt rooted at you。Now。Again， I'm overloading the word size here。

 this is size as rooted subt of T， this is not size of some subt in the data structure。

 this is in the abstract tree。So it just means the number of descendants。Of V or U。In the tree。Okay。

And so just like I can say， I have a preferred child， every node has at most one heavy child。

If I have a million nodes in my subtre and that child over there has 700，000 nodes in its subtre。
...
And the reason that I can do this。there's a simple， simple claim it's easy to solve， easy to prove。

Any。Root。To leaf。Paath。In T has at most log n light。Edges。Okay。U。Because。If if I think in my head。

 whenever， okay， I'm going to walk down this abstract treat T that I'm representing。

 starting at the root。And at all times， I imagine that the size of the subt rooted that node。

 the number of descendants is written on the node。So whenever I step from a node to one of its light children。

That number drops by at least a factor of two。But at the beginning， it's N。
...
Proof。This is basically the definitions of light。And。login。Okay。

 log ends the number of times I can divide starting to then divide by two until I get to some number less than one。

嗯。Okay。again， this is not the same decomposition as the preferred path decomposition that we're using to define the data structure。
...
Every edge being light， that's still consistent with this claim。
 ...
 but even if the trees really unbalanced。I'm only ever going to need to on any path need to traverse log n light edges。

 and so it's the heavy edges that I really care about。Okay， so。The number of。

I'm going to write it as。The number of edges that become preferred。This is。
...
There is one preferred child pointer at the very bottom that I set to null。
 ...
 and so that's that's a case where。Um。There might be a preferred edge before and there's not after。
...
Whether an edges heavier light is fixed， accesses don't change whether an edge is heavier light。
...
Of the number of heavy edges。That I make preferred。This is， well， every。

 every edge that I make preferred。Almost all of those edges are later but going to become not preferred again。

Except for whatever edges are heavy and preferred at the very end。

So I can write this as to some over all accesses。Of the number of heavy edges that become not preferred。
...
Because at the end of time， there are at most n minus1 heavy preferred edges。

 because at the end of time， there are at most n minus1 edges。

Every other time I make a heavy edge preferred， it's later going to be made not preferred again。Okay。

 so really。This is the quantity。That I want to analyze。

So this is where I'm doing the amortization trick is through the summation。

 I'm saying I want to amortize the number of heavy edges that become preferred。

And I'm going to do that basically by charging every one of those changes to。

future operation where it becomes not preferredvert again。

And now I just need to know how many heavy edges become not preferred。嗯。But now。诶。See about this。
...
Okay， so I need to add one for that possibility， the note I access had a heavy preferred child and now doesn't anymore because it doesn't have any preferred children at all。
...
So this is at most one plus login。Okay。No。嗯。What about？Cut。Now。

 CA is a little bit more complicated than an access。

Cut is a structural change to the tree that I'm representing now cut always when I cut an edge。

 I start by accessing the parent end of that edge and so I'm already going to count that in the access part of that I've already analyzed。

 but then I'm going to remove this edge from my represented forest。Now。Nodes on that excess path。

When I remove this might change。From heavy to light。ok。So。嗯。So， now。呃。No， sorry。Some。😔，Edges。
...
Which means now it's possible that my right arm is holding most of my descendants。系。嗯。Okay。

 so let's think about what we're counting here。Is counting。Preferred edges that stopped being heavy。

Right， so this is a change in。I should really think of this as。The number of。Edges。That stop。Bing。😔。

Both。Preferred。And heavy。It's not that the heavy edge became non preferred。

 it' that a preferred edge became light。It still goes into the analysis in essentially the same way。

And so but now it becomes a light edge on the access path and well I know there are only log n light edges on any path。
...
For a cut， it changes by most log n and for a link， it doesn't change at all。Again。
 ...
 in an amortized sense。Okay， so this is all going to。U。You know， the summary is you know。

 this thing here， this term that I had in the amortized analysis。嗯。

This is the number of edges that change from preferred to not preferred。For an access。
...
It's the right way to think of it because I already have a login sitting next to it。
 ...
 the status of only log n changes log n edges changes whenever I do any operation in one of these ST trees。

Yes。P is。Yes， that's supposed to be login。Okay， so this is the， you know。

 the three kinds of operations that I want to perform。Access big of log， cut big O login link。Well。

 it not counting the access that you need to do on each of those nodes before you linked them together。

 no additional cost， yeah。I there should be something over。Yes， that is absolutely correct right。
...
And I need to consider both of those in the summit。O。So。This is most of。The analysis of ST trees。
...
I talked about just unrooted， unordered free trees。
...
You know so。Generically。I'm going to have operations that look like this， I pass in two nodes。

That belonged to the same tree， and I want to do something。

 I want to retrieve some information about the path between those two pets。Now， now let's just。

Let's say the abstract。ree。T is unrooted。But。The data structure assumes the tree is rooted。

 so I'm going to for the convenience of building the data structure， choose anode to be the root。

So the way that this path query is going to work is basically first。诶。I'm going to。Access。You。
...
And then I'm going to make you the root and then I'm going to access V and then everything that at this point that I want to be。
 ...
 all the nodes that are on the path between U and V are going to be collected into one nice rooted sub inside。
...
And maybe do a constant more operations， but make root。Is the。

The part that I still need to define here。Okay， so let's。Imagine， okay， here here I've defined。
...
There's another path tree here and maybe know somewhere down here is V。Right。But at the root。
...
Just not that it really matters， but but just to I need to。Standard one way or the other。嗯。Actually。
...
Now， if I want to make you the root。The way I do that。As。To make。You the root。I reverse。Pa。
...
That all edges are pointed away from now the new root U， and even if I've got other stuff。

 not not just hanging off of T， but hanging off of U or hanging off of R。

 all those other edges are still pointed the right direction。😡，Away from the root。Okay。

 the only edges that have changed direction when I change what finger I point， you know。

 what node I put my finger on and say， this is the root node are the edges along that path。Well。

 I've conveniently collected。All of the nodes along that path into a spplay tree in order by depth。
...
So this is set the reverse bit。In the top path tree。哦。So this now goes reversed。

And then lazily update it。So。呃。If for any other reason。I need to。Touch the root node。
...
so this is just lazy propagation exactly the way that I described for oiluler to trees for the way you would say update this all the you know add five to all the nodes in the subtree would say。
...
Because now the notion of what a ased is has changed。
...
The data structure is literally said a bit done， but you have to pay log n in potentially when you do that to pay for changes in heavy light labeling interventions。

Right so。Again， this is log N Em。Now when I want to do。This。啊。Okay。

 so that's making something a root。But now after I've done the， make you the root。And then。Access V。

Then at the end of that。응 그래。Duplicate。Right at the end of that。Sorry， let me u。
...
It's just whatever happens to be the last thing it was played。Am。But now。If， for example。
...
Then I just look it up at V， look up the answer at V if I want to do a path add。

 I want to add seven to every node along this path。

Then I recorded every node a value that I want to add to all of my descendants。

 and I just write that value at V， and again I need to lazily update whenever I need to do anything in this path tree。

 first I need to clear all of the lazy operations that haven't performed along whatever the new search path in that sp tree is。

Okay。So after doing this。Querries。You look。At the sort of summary information。Stored at V updates。

A sort of lazy。Just like。Oil to trees。And so that is all the， you know， add things along the path。

 find the maxle on the path， negate everything along the path。

 same the same lazy update strategy at the end result of doing that。呃。

Access finder access is that that entire path is now collected into a subte of the ST tree。

 it's not a subt hanging off the bottom， but rather it's a subte touching the root。有。I不 yeah sorry。

So， sorry， yeah， each no。In data structure is going to store the information tree Yes。

 every every node is going to store information about its descendants in its path tree。

 not its descendants in the whole structure。大都是方。1。That's correct。
...
Byiceplay。I think there was another question over here。Or somebody was stretching。Okay。All right。Um。

The last thing I want to mention is。You can also do least common ancestor queries。

So if the tree does have a root。What you're asking is given two nodes。
...
That's this note here。嗯。You can think of it as make W the root and then ask for the least common ancestor of UN andB。

Um。This can also be done in。Log in amortize time。So I'll do join here。嗯。Access U， make root you。

Access V。Access。W。And then one more access。Now， why am I flopping back and forth like this？Okay。
...
And so is most of this power。Because I only change the I do change one preferred pointer hanging off of the node that I'm looking for。
...
I will start talking about applications of。These dynamic foresting where all of these operations that I talked about actually show up。

And we'll see where we go from there， that's probably going to take me。A few more lectures。

 so the one I want to talk about tomorrow。Is involves shortest paths and planar graphs。
