# UIUC《高级数据结构｜CS 598 JGE Advanced Data Structures — Spring 2025》中英字幕 p06 -17-Feb 06： Dynamic optimality, tango⧸multisplay trees.zh_en -BV1kWFGzsEmN_p6-
...
Oh。It extra minutes。To get started and thanks for coming， despite the lovely weather outside。i。

Let's see， before we get started on the content， a couple of quick admin announcements。

 homework zero solutions are out。嗯。Whether or not there's ever going to be a homework one。

Depends a little bit on my ability to get homework zero graded quickly。I'd say there's probably if。

At best of 50，50 chats of there being another homework。嗯。We'll see how it goes。
...
So you still have the time that same amount of time roughly between proposal and presentations。
 ...
 I did not change the paper chase deadline and those you've been waiting I'm expecting to put out the。

The instructions and list of papers for the paper chase deadline over the weekend。

 so then you'll still have about three weeks before that will be due。Okay， so。Project presentations。

Those project talks。are。A week earlier than。Originally planned the。
...
Get things out of the way， but that deadline hasn't changed。
 ...
 it's just the presentations that have changed。Okay。Andng。Any。Admin or logistical questions。Yeah。这这可。

Yes。So it used to be。Like in the first the。Project talks used to be like May 88 through 11。
...
And there are all sorts of lovely properties that these particular self adjusting binary your search trees have。

 so one that we saw is you know get log in amortized time for any operation， but you also get。
...
So this is the search target。And this is the。Static。Finger。

 just one of the items and the static finger is not something that the data structure knows。
...
This is kind of a spatial or temporal locality of reference。So sort of automatically。
...
So this is like spatial locality of reference， if I'm looking。
 ...
 imagine that I'm thinking of my search base as a sortrded array。
 ...
 and again， none of this is using any modifications to the data structure itself。U。

Then there's this conjecture。Called the。Dynamic optimality conjecture。Which says that the。

That says display trees。Or。Constant competitive。Against。Any。Let's say just call it dynamic。BST， even。

If that other tree can see into the future。Okay， so what this means is for any sequence。

Xs of searches。The time。Using the Sp tree to search for x is big O of the time using the optimal binary search tree to search for x。

So X here is a sequence， so 31415 means search for three， search for one， search for four。

 search for one， search for five， and so on， and you're adding up the total costs of all those searches。

Using display tree。Which can't see into the future。

And you're comparing that to the running time of a hypothetical， optimal binary search tree。

Data structure that can do the same kinds of adjustments when it does searches to make future searches cheaper。
...
For any search sequence。If you use a tango tree or a multiplay tree to search for that sequence。
 ...
 one thing at a time， not looking into the future， the total running time of that those searches is within a factor of log log n。

Of the optimal binary search tree， even if that optimal binary search tree is given the entire sequence in advance。

嗯。Now。We're wandering now into the realm of what could either。
...
So when you bring a page of memory off disk。IntoInto RA or a cache line out of your Ram into say。

 your L1 cache。You have to have a policy for which page or which cash line you're then going to eject because you have to make room。
...
You don't know the future。But what you can argue is that if you use some standard algorithms like least recently used or first and first out。
...
Is I need to understand exactly what the rules are for what i'm comparing against and I need to understand the the cost model for that thing。

 This is sort of the online algorithms framework I have to decide what qualifies as an algorithm for purposes of。
...
You don't actually try to prove a factor of three versus the optimum。
 ...
 you try to prove a factor of three that's something that is less than the optimum。
...
Um， so。Wilbur stood out and say， okay， look。We want to allow we want to consider only binary search trees。

 okay， so lets let's set up one set of rules， right？The data structure。诶。Must be。
...
A binary search tree now this rules out some of more interesting data structures that we'll see later that if I know that my keys are integers。
 ...
 but I'm violating this model that it's a binary your search tree。UmAnd。I can do arbitrary rotations。
...
The rotations are what take up all of the time。For purposes of defining the model。
 ...
 obviously everything else isn't completely free， so the time you're getting here isn't really the running time。

 it's just a lower bound。On the running time。系咯。Now， the interesting thing here is。啊。You know。

 there's a second model that you can imagine， which is maybe a bit more realistic。Is that okay。

 so the data structure。哦。Is a binary search tree？And now I have one。Pointer。I'll call a finger。哦。

Into the BST。And I'm allowed to do。One of four things。To that pointer。
...
诶。So I'm allowed to perform any sequence of these operations。嗯。Each of these by definition。

 takes well。Actually， I shouldn't say order one。Each of these you know is one step each so the the running time is the number of these operations and the requirement is that。

During the search， the finger。During the search for X， the finger must。Touch。X。Okay， so I have a。
...
I walk down the search path to X and then I walk back up to the root。
...
This also completely describes red black trees， AVL trees， troops， scapegoat trees。
 ...
 if you're really careful and s trees。So display trees， remember you start at the root。
...
嗯。Um， so here。Right， time。Equals the number of operations。
...
The asymptotic notation， remember， is something about limiting behavior。
...
So this magical， I can do remote rotations over there。
 ...
 doesn't really help me and the intuition behind this。Is that？We do remote。Rotations。Lsly。
...
If those rotations are somewhere off the search path， like those。I will say， oh yeah， yeah。
 ...
 I did that and I over here that I should do that later。

Okay so I'm performing rotations that touch the search path immediately。

But rotations that are far away， I'm not going to touch， and if this is the last search。
...
And you want to say look at its right child， you need to know that its right child is correct。
 ...
 so any rotations that involve that node or its right child now need to be performed in chronological order and any rotations depend that those depend on need to be performed in chronological order。

So what this means actually。Is during the search。You're kind of reconfiguring a tree that includes the root and the search target。

And you're doing that all using these local moves。系。So there's a third equivalent model。Which is。

During the search。I， I pick。A subt tree。S containing。Both the root。And。And X， and by subtree。

 I don't mean a node in all its descendants， I don't mean a rooted subte。

 I just mean a connected part of the binary search tree。系。And then I'm allowed to reconfigure。S。

Arbitrarily。Meaning this subtree S is a binary search tree over the sum subset of the notes。
...
Is that display trees？If you measure the running time of display tree， the way you normally would。
 ...
 said number three is prevalent to the previousview to like， how is the sign of the number of remote？

So I've left out intentionally I've left out a couple of technical details here。

 so it wouldn't be accurate to say we've shown this as Wil burursh this， but one of the things。啊。

That's not too hard to prove。Is that you without loss of generality in any of these models。
...
If you don't want it to be at the root， then the beginning of the next search。
 ...
 you just push X back down to where it used to be。If you make this assumption。Then in Model two。

 your finger is always starting at the root。And it's wandering down into the tree。

 touching a bunch of stuff and then coming back up。So the total amount of time you're spending there。

Ignoring rotations is proportional to the size of some subre。 You're just wandering around sub subre。
...
So that shows that， you know。I'm waving my hands a little bit。
 ...
 but that's the basic idea between neo equivalentvalence of models two and three。

And then the equivalence between models three and one is this， let's wait。
 ...
 and then that grows more of us。Good。No。Before I go one。Talking about binary search trees。

I want to take a detour。Very briefly。And talk about。The problem in homework zero。Okay， so I said。

'm going to I want to convince you。That n log n is the best that you can do。
...
系。So this is an argument of Fredman。Um， so this is。Homework 0。2 lower bound。Okay。So。

I want you to imagine a slightly more general model of dynamic binary trees。Where you know， to prime。

I'm allowed。To。At every step。I can。Move left。Move。Right。Move。U。Rotate up。Or。Swap。The children。Okay。😊。

Um， now。I'm insisting on locality here。Okay， so。And let's suppose I have let x be some。Permutation。
...
Or to get to the search target I。That's free。The only thing。
...
Report root as another operation， I can sort of when when my search search target reaches the root。

 I can say， hey， I found I， I found。I found the the。I guess I should say x subi。

 the I thing in the sequence， hey， I found the I search target。喂。

So if I look at the trace of this algorithm。To trace of the algorithm I'm writing down a sequence of symbols from a six character alphabet。

 left， right， up。Rotate。Swap， root。Okay。So the trace of the algorithm is a string。Over。Alphabet left。

 right， up。Rotate。Swamp。Root。You make up whatever symbols you want。

Corresponding to these six operations。Okay。Now notice that。
...
The sequence X of items that I should be searching for。Okay， whenever I just。You give me the trace。
 ...
 I run the algorithm and whenever you say hey report the root I write down whatever symbols the root and that will recover my permutation X。

 so this means that I'm encoding。😡，Per mutation。Of N。Using。S symbol。Alphabet。や。天议。不ばい。

He is our current tree。It's not necessarily a binary research tree。

It may never be a binary search tree， it might be a binary search tree。Okay。So the trace， again。

 assuming I start with some fixed tree。That I know in advance， the trace encodes this permutation。

Right right just。Execute the algorithm and then every time I say， hey， I found something at the root。
...
But the length of the trace is the same as the running time to actually access X。

whatever algorithm you're going to use， there is some sequence of accesses and no matter what algorithm you use it no matter what tree you start with。

There is some sequence of accesses， some permutations。

That's going to require n log N time in this model。嗯。On the other hand。U。For any X there。Is a。
...
There is a starting tree such that the running time of or sorry。
 ...
 I should say the optimal running time for x， starting with that tree is only order N。So for example。

 if the bad permutation for your algorithm in your starting tree。Is one， two，3， four，5，6， seven。

 eight9， et cetera。Then I will prepare a tree， which is just a linked list going down to the right。

 one， two， three， four， five。And my access sequence is， hey you reported the root。Go to the right。

 rotate， report at the root， go to the right， rotate， report at the root， go to the right， rotate。

 report at the root。If x is some other permutation， well。

 I'll just fill that permutation in on the right chain anyway， right so this is just。X1。Next two。

X3 down to xn。ok。😊，Now the magic step。Is。Oh。The observation， the sort of generalization of。

Of Willard's proof， you model number one is I can search and rotate anywhere。And。Swamp。Anywhere。

And this is you can simulate an algorithm that works in this model where I can do rotates and swaps anywhere at all in this local model by delaying rotations and swaps until they actually touch the thing I want to search for。

Okay， so this means that if I start with my initial tree t0， there's another starting tree t1。

 so t0 and t1 are omega n log n rotations。And swaps。Aparted。

OkayI'm furiously waving my hands here about the details。Um。

 but u and this is the part of the proof that I understand the least。
...
In fact， I never need swaps， I only need a linear number of rotations。T0， which is a BST and T2。
 ...
 which is another BST。Are only。A linear number of rotations。Apart。For any BSTT2。

So all binary search trees are in a little ball of radius order N。But there is something out there。

 distance M Logan。So in order to get out of this little ball and to get all the way over to T1。

 so that means the the distance from。Any。Binary search tree。To T1 is omega n log n。And so T1。

Is the worst case input？To the problem in the homework。So you can't be done it。
...
I think。Frednant does a slightly differently， it' first make it a balance binary new research treat。

And now with a logarith number of operations， I can move any node anywhere I want。
...
Do you get a bigger then？Okay。So。There's a big wide open gap in my description。
 ...
 the total number of strings I can get is only six to the L。

So I need 6 to the L to be bigger than n factorial。

So I need L to be bigger than log base6 of n factorial。That's真毛病。

This is essentially the same argument that if you want to sort an array by doing pairwise comparisons。
...
The sorting permutation。But each comparison only has two possible outcomes。
 ...
 so if your running time is L， you can only encode to the L different permutations。

So2 to the L needs to be at least in factorial。有哦。So， of course all。

 the lower down sorting argument is similar to this because you can construct a language where the two yes are left that greater than you only have that's right。

Yeah。How do we go from like this kind of moving okay enco this permutation argument to transforming 30 to 31 like okay so so this is this is the sort of the sort of magic step so what this says is you have some algorithm。
...
and perform rotations and swaps where it is， and if it's at the root， says， hey， I found number five。

Within that model， you must do N log N。Operations to get from。To execute some access sequence。

 no matter what you start with。On the other hand， for any access sequence。

 there is a tree that only requires order and operations。So in particular， I can't。

Do rotations and swaps turn it into that tree and then only use that tree。

Because then the transformation would need to take N log N because then the rest of the algorithm would only take linear time。

You still have a question。So like I'm just thinking like how far can get trapped by is like。

This plus like planter trees， I guess， suppose like in the year 2040 some guy comes around like a data structure and meet claims like。

 you know， we can do sorting or we can arrange the data here to have some kind of as a well ordering in a of event or you know。

 yeah， you know then。As long as there's like。A constant number of operations that we restrict ourselves to would the same truth apply there as well。
...
I can get from any binary search tree to any other using only a linear number of rotations。

And only about2 n rotations。Well， I get the same kind of encoding thing in Wilbur's local dynamic binary search tree model。

 I can recover the binary search tree by looking at the trace of an algorithm。Encoded， in some sense。

 I can encode。Any binary search tree using a string over this five character or four or five character alphabet。

But the number of search trees is only the emph catalin number， which is about four to the end。

So I only need。Loob base four of four to the N。Symbols to encode。My favorite binary search tree。
...
Let me go back to like。Wilbur's results about。呃。Dynamic binary search trees。嗯。I'm gonna to。Gove。
...
U。And。This is a lower bound that。Wilburt derived for the number of operations required to execute。嗯。

An access sequence on any。Dynamic binary research trip。So this takes a couple of steps to set up。嗯。

But you'll see some similarities with the thing that I just showed you。 Okay。

 so I'm going to fix a reference。ree。Which I'll call for historical reasons P。
 ...
 the argument goes through regardless of which binary search tree you start with。

I should say that only。Sorry， let me actually do a little bit of setup here。

The task is that we're given。A。Per mutation。Acs of。The integers from1 to n。And we。We need。To。Access。

Next1。Then x2。

![](img/7d3e24504a192f587f4a695fd08c718d_11.png)

Then and and so on all the way up to XN。And we're interested in the total cost of that sequence of event accesses。

In this model where I have a finger that moves around and when I'm done。

 I have to end up at the root with the right key。Com on。
...
And on the right region is the set of nodes。In the right subre。With respect to P of y。
 ...
 so this is R of7。Okay。U。Now， if I had 8，9， 10 and someone hanging off eight。

 those would also be an R of 7， but if I had if I had9 in 10 down here and then 11 up at the root。

 that 11 would not be part of R of7。Okay。So。I want to imagine。Now。

This perfectly balanced tree or this reference tree is kind of every node in this tree is monitoring the search sequence。

And as the numbers go by in x。Every so often it goes hey， that belongs to my left region， hey。
...
Of these two regions。OkaySo anything in the subre rooted at y， y is going to go， hey。

 that's one of mine。And either they're going to raise their left hand or they're going to raise their right hand。

So the interleaving bound。For x with respect to y is the number of。Alterernnations。Between。呃啊。

The left and the right in this subsequent x to the y。Okay， so in this example。

 if my search sequence is three1，5，4，8，62 seven。Noode7 goes， well， I don't care about three。

 I don't care about one。 hey， five is in my left subree， I don't care about four。

y eight is in my right subte， Oh， six is in my left subre， I don't care about two seven is me。

 but I'm going to break ties to the left。So in this case， the interleaving bound is two。Okay。

I should really say here。In even bound of x7 is2， this is x。And then finally。

 the overall interleaving bound for x is the sum overall y of the interleaving bound of x with respect to this keyY。

And thelemma。Is that the optimum search sequence for x。

 the length is at least half of this interleaving bound。Minus n。
...
And the intuition here， again， I'm not going to walk through the details。

 is essentially every time node y in the reference tree switches from raising its left hand。

 raising its right hand， there's a particular node that had to be touched。

Either during the search for the left thing。For the search for the right thing or somewhere in the middle。

 there's a particular node that we can identify that within that sequence of operations must be touched。

 and that node is different for every node why。So the， you。

 I'm actually counting distinct things that add up to this interle， lower bound。 Again。
...
Because it's going to show up in the design of the A trees and multiplay trees。Okay。
 ...
 so does that anyone everyone kind of get？At least what the lower bound is saying。

You should not necessarily believe the lower bound， I haven't really given you a proof。

 but do you understand what the lower bound says？O。So。Cango trees。And multiplay trees。

Are essentially the same data structure。Multi sp trees turn out to be slightly better in a couple of different ways。

嗯。呃。Tano trees， on the other hand， actually give you worse case bounds instead of amortize bounds。So。

 but morally they're equivalent。Okay， but okay， so again， I'm going to be doing。Accessses， searches。

Only。Okay， so I'm not going to talk about insertions and deletions。In this proof。

 multip trees can work with insertions and deletions as well， and hopefully it'll be clear。On how。

Okay， so I have， again， some fixed set of keys， which I'm going to imagine without loss of generality or just the intros went through it。
...
N is 15， then I can draw my perfectly balanced tree。This is P， it's fixed。
...
Which is a pointer to its favorite child。Now obviously， if it's a leaf。
 ...
 it doesn't have a favorite child， because it doesn't have any children， but so each node。In P。

Points to its preferred。Child。Okay， so one of these children， I just like more than the other。看。
...
This is a preferred path。This is a preferred path， this is a preferred path。
 ...
 this 13 by itself this you know。So there are preferred paths which vary in length from zero to log n。

There is， in fact， one path。One preferred path that goes all the way from the root down to its leaf is just what followed the root's favorite child。

 And then that node's favorite child。 And eventually， I'll get down to a leaf。

Now the semantics of this。Is that this always。Points。Toward。The most。Recent。Search target。UmOr left。

Right， so what this means is the last time I did a search in the subt rooted at four。😡。

The target of my search was either five， six or seven。So my right child is favored。

 preferred because the last time I needed to do any work， I needed to do it to access the right sub。

On the other hand， what 12 is saying the last time anybody looked in the subs rooted at 12。

 the search target was 9，10， 11 or 12 I like my left child because either the last time I did any work。

 I had to something down to the left child or I was the search target。see。

 this is kind of exactly the same setup as Wilbur's lower bound。And。Now。

This is sitting in the background， anytime you do a search you update these preferred child planers。

😡，so these are updated。After。Every。Search。Okay， so。And moreover， the path。To the last。Search target。

The most recent church target is preferred。So I can look at this tree。

And I can tell that seven was not the most recent search target because if it were six will be pointing to seven。

 and I know that 13 was not the most recent search target because if it were eight would be pointing to 12 and 12 would be pointing to 14 and 14 would be pointing to 13。

😡，Okay。So what does the tango tree do？The idea here is I'm going to。Represent。Every preferred path。

Using。A binary search tree。Now this is a strange thing， so I'm going to have， for example， over this。

Set of four nodes。I'm going to have some sort of binary search tree。That represents that path。

And then for the next set say9，1012， I'm going to have another binary search tree that represents。

That。Path， and these are going to be connected together into one giant binary search tree。Now。

 why do I do this？The idea is。I'm sorry， I am running out of time so I'm going to have to come back to this。
...
But each of those component trees has the most log n nodes in it， so its height is log log n。So。

 the total amount of time。That I need。To do a search。Is log。Log n times the number of changes。2。
