# 斯坦福大学《算法（分治／排序／搜索／随机算法、图搜索／最短路径／数据结构、贪心算法／最小生成树／动态规划、最短路径／NP）｜Algorithms》中英字幕 - P118：43_03_05_动态规划原理.zh_en - GPT中英字幕课程资源 - BV1Rx4y1U7sZ
...
Hey， so guess what？ We just designed our first dynamic programming algorithm。
 ...
 Now， I've deferred articulating the general principles of that paradigm until now。

 because I think they're best understood through concrete examples。
...
So the key that unlocks the potential of the dynamic programming paradigm for solving a problem is to identify a suitable collection of subproble。

 and these subproblem have to satisfy a number of properties。

In our algorithm for computing MaxW independent sets and Pa graphs， we had n plus1 subproblem。

 one for each prefix of the graph， so formerly our IF subproblem in our algorithm。

 it was to compute the MaxW independent set of G subI of the path graph consisting only of the first I vertices。

So the first property that you want your collection of sub problems to possess is it shouldn't be too big。

 You shouldn't have too many different subproble。 The reason being is， in the best case scenario。

 you're going to be spending constant time solving each of those subproblem。

 So the number of sub problems is a lower bound on the running time of your algorithm。 Now。

 in the Max weight independence set example， we did great。

 We had merely a linear number of sub problems。 And we did indeed get away with a mere constant work for each of those sub problemsble。

 giving us our linear running time bound overall。The second property you want。

 And this one's really the kicker is this should be a notion of smaller sub problemsble and larger sub problemsble In the context of independent sets of path graphs。
 ...
 it's easy to infer what the solution of the current subproble is。

 That is solutions to previous subproble are sufficient to quickly and correctly compute the solution to the current sub problemble。

The way this relationship between larger and smaller subproblems is usually expressed is via a recurrence。

 and it states what the optimal solution to a given subproblem is as a function of the optimal solutions to smaller subproblem。

 and this is exactly how things played out in our independent set algorithm we didn't even have a recurrence it just said that the optimal value。

 the max weight independence set value for G sub I was the better of two candidates and we justify this using our thought experiment。

 either you just inherit the max weight independence set value from the preceding subproblem from the I minus-1 subproblem。

 or you take the optimal solution from two subproblems back from G minus-2 and you extended by the current vertex v sub I that is you add the I vertices weight to the weight of the optimal solution from two subproblem back。

So this is a pattern we're going to see over and over again。

 we'll define subproble for various computational problems。

 and we'll use recurrence to express how the optimal solution of a given subproblem depends only on the solutions to smaller subproblem。

So just like in our In at example， once you have such a recurrence。
 ...
 and you use your recurrence to just fill it in moving from the smaller subpros to the larger ones。
...
So the third property you probably won't have to worry about much。
 ...
 after you've done the work of solving all of your subpro。
...
This property is usually automatically satisfied because in most cases， not all， but in most cases。
 ...
 the original problem is simply the biggest of all of the subproblem。

 Notice this is exactly how things worked in independent sets。
...
Now when you're trying to devise your own dynamic programming algorithms。
 ...
 if you nail the subproblems， usually everything else falls into place in a fairly formulaic way。

 Now if you've got a black belt in dynamic programming。
 ...
 So rather in the forthcoming examples rather than just plucking the subproblem from the sky。
...
So perhaps you were hoping that once you saw the ingredients of dynamic programming。
 ...
 let's go ahead and turn to Richard Bellman himself he's more or less the inventor of dynamic programming you will see his Belman Ford algorithm a little bit later in the course So he answers his question in his autobiography and he he talks about when he invented it in the 1950s and he says those were not good years for mathematical research He was working at a place called Rand he says we had a very interesting gentleman in Washington named Wilson。

 who was the secretary of defense and he actually had a pathological fear and hatred of the word research I'm not using the term lightly。

 I'm using it precisely his face would suffuse he would turn red and he would get violent if people use the term research and his presence。
