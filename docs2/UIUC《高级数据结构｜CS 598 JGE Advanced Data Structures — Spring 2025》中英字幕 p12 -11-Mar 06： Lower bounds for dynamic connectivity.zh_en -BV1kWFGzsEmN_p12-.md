# UIUC《高级数据结构｜CS 598 JGE Advanced Data Structures — Spring 2025》中英字幕 p12 -11-Mar 06： Lower bounds for dynamic connectivity.zh_en -BV1kWFGzsEmN_p12-

![](img/24f1fe3535e5a1b7e682d315b19930ac_0.png)

Okay。嗯。So again， just a logistical reminder， you still have some time to get your paper chase in。
...
Data structures for dynamic trees or of dynamic forests where you。
...
You're maintaining you have a fixed set of vertices and the operations you're allowed to perform are at an edge between these two vertices。

 delete an edge between some pair of vertices， and then ask whether two vertices are in the same connected component。

Of the resulting graphs。Now， if somehow you could guarantee that the graph was always acyclic。

 this is equivalent to what we did for dynamic forests。

 but more generally it's not and we ended up describing a data structure that used amortized。

Log squared end time for the updates and log N time for the queries。Um。
...
Worst case timeouts where you're not allowed to average over the operations in the sequence of the lifetime of the data structure。
...
To how quickly these data structure operations can happen。嗯。

So I'm going to be working in the realm of lower bounds now a lot of lower bounds are ultimately information thetic。

So for example， if the query that you want to support is。U。呃。

Find me the case largest thing in this array event items。嗯。There are n possible outputs。Namely。

 you know， the K largest thing might be located at index1 index two， index3 up through index N。

And so your。Query algorithm must be able to uniquely identify one of these integers from one to n。
...
Then it must make log n of those comparisons。Because each comparison is essentially generating a bit。
 ...
 and I need to generate a unique integer between one and n。Now。

 I'm not saying necessarily have to generate that integer in the standard binary encoding。
...
Similarly for sortdium， if I'm sorting by doing comparisons， the output of a sorting algorithm。
...
Again， necessarily doesn't necessarily mean that you need exactly the same number of bits for every permutation。

 but the average permutation is going to need at least n log n bits。

Because log of in factorial is then log again。So this is， you know。ultimately。

Information throughout it。Because this is to first approximation， the only tool we have。

To prove that things are hard。But。I don't want to make any assumptions about what the algorithm is doing。

 I don't want to assume a priori。😡，ThatThe decisions made by my data structure。Are comparisons。

Because algorithms can do much more interesting things than comparisons。

 I can build large tables of things and use the value stored in the table over here an index into the table over there。

 that's not something that you can in constant time。

 but it's not something I can encode using a constant number of you pairwise comparisons。

So I'm going to work。In a much more general model of computation。嗯。Called the cellll probe model。

And this was originally devised。By NL。I believe is the only Turing award winner to graduate from the University of Illinois。
...
And there are some circumstances where the answer is no， because you can actually like if I。
 ...
 if I know， for example， that。Well， I mean， one easy out is under some circumstances。

 I can use Haing。So it's like， yeah， it's not entirely clear that the answer is no。

 it's sort of intuitively if I'm doing， it's certainly if I'm only doing comparisons。

 the answer is yes， that's the best you can do。But in this more general model， not so clear。

 and this is also I thinking。Sor of one of the sources of the model of computation that people actually use in the 21st century to formalize what a computer is for purposes of thinking about algorithms called the Word RAM model。
...
But at least the one aspect of it that I want to socialize now is what memory means。Okay。
 ...
 so in the cell probe model。Each。U。Memory address。Stors。W bit word。
...
So the idea here is。I need。Words to be learned enough。That I can access。Inputs in constant time。
...
And then I go out to memory and I grab A of I and that takes constant time if my memory address if my word size were too small I might need to use multiple words just to encode that address。

 I might need to use multiple words just to do something as simple as for I equals1 to n。
...
It's really bad if you let W get or let's say， really unrealistic。If you let W get too large。

 so theta log n is a reasonable assumption and the lower bound is necessary and the upper bound is。

Fits with experience。系。But these words。啊。Could be data。And or they could be other addresses。

Which means that memory。Has。Two to the W cells。Okay， so memory use finite。Now， again。
...
Some sense depends on the amount of space that your algorithm is using and for algorithms that run in super polynomial time。
 ...
 two to the W needs to be large enough that linear size data structure can actually fit in memory。嗯。

But so that2 to the W really just means polynomial and N。嗯。Time。Is the number of reads？Plus。
...
The only thing I care about is operations of the form， here's an address。
 ...
 go give me the content of thats up。And here's an address and a word。
...
Because algorithms need to compute things sometimes。But it is because it bunder counts。
...
Then that implies that any algorithm that kind of fits within this paradigm must take at least log end time。
...
There's this。啊。Problem called sorting x plus y。 so I give you a set x and I give you a set y。

 they're both represented by sorted arrays I want to。Build and sort it。

The set containing all sums of one element from x and one element from y。Okay， this can be done。

Yin a。N squared log n time by brute force。Right， just。For all little x and x for all little y and y。

 little x plus little y into an array， sort that array。はい。U。On the other hand。

 if you're very careful。诶。You don't need that log。If all you are counting is how many times you were comparing。
...
And。Fraashku。Think this is around。2006， I I exactly sure。嗯。Any dynamic connectivity data structure？

Requis。At least login。Time。😔，Per operation。Where n is the number of vertices in the other line graph。

 and this is even if you allow amortization， even if you allow randomization。

 and even if G is a bunch of dis paths。对。So。The randomization actually comes in because the lower bound argument is itself randomized。
...
嗯。All right。So。What's the the。I here。Is。I'm going to construct。

For purposes of proving the lower bound。I'm going to imagine first that I have a sequence of capital and operations that I want to perform。

And these are these operations come in chronological order。
...
UOpers， let's call them you know， a1， a2 up through a capital N。
 ...
 let me assume that this is you know a perfectly balanced binary tree I'm going to look at a particular node V。

And I'm going to consider。The relationship， but how information in some sense。

 flows through this vertex V。So each of these operations is going to read from some number of cells in memory and write to some number of cells in memory。
...
That are at the same addresses as reads。In the right century。嗯。So if you。

 I think one thing can translate to a non。Is to imagine something called a chronaogram。So， every。
...
Address。Remembers。When it was。Last。Written。So if I'm in the middle of executing one of these operations in the right subre。
 ...
 and I read and address in memory。I kind of care- I'm interested in that read if the last time that memory cell was written happened in the left subject。

嗯。So。Yes。In order answer to correctly execute the operations on the right。

I need to know what was written。Into those addresses on the left。So there's sort of information。

That explodes。Through the is sort of reads。In the right。V。That were last。Written。In left of V。

And I have to know the values that were written over here in the past。

When I'm in the past of V when I'm executing operations in the future of V。系。嗯。And ultimately。

 the argument， the lower bound argument is going to say， okay。

A certain amount of information is it needs to flow across this barrier from the left side of V to the right side of E in order for me to correctly answer the queries and perform the updates on the right side of V。

That's going to give me a lower bound on a subset of the operations。

The subsets of the reads and writes。But now notice。For every read and write。There's， you know。
...
Say at time T， I say， when were you last written before I go any further， you goes， oh。

 that was a time T prime。I'm going to charge that operation to the least common ancestor of T& T prime。

And so this ensures that none of the reads or writetes are going to be counted more than once。no。
...
Fine， I can be again， liberal for purposes of proving a lower bound。

 but then ultimately the lower bound for the total number of reads and writes which was going to be the sum。

Of the readw writeite pairs that cross through each node， summed over all the nodes。Okay， so。
...
To be executed correctly。嗯。So。呃。Also， I should mention that the operations that I'm going to describe are going to be randomized。
...
My vertices are always going to be。This nice root end by root and grid。My entries。Orre or。诶。Perfect。

Mushins。嗯嗯。Prom。So for example， I might have this and then。This， and then。This。And then。诶。This。系。But。

This is always what my graph is going to look like。But between operations。

 this is what my graph is going to look like。So。Let's see。呃。Pm X。Then codes a permutation。I sub x。

Over。嗯。The numbers went through squared n。And in particular， the entire graph。
...
Encodedes a permutation by composing the permutations inside each column。So if I write one， two。
 ...
 something like that on the far right。开。Now， I'm going to define what I'll call macro operations。
...
So I'm aggregating them into these so called macro operations one of them is。Update X pi。
...
Okay。😊，The secret macro operation， so this is。Change。The operation column X to。The given perm。对。
...
P pii at column x composed with pi column x minus1， all the way down， composed with pi 1。So I verify。
 ...
 encode the following permutation of the integer's through five？系。Update。
...
😡，Similarly， verify。Hope it's clear that that can be executed by checking square to n pairs of vertices to see if they're connected。
...
So if I can show that these operations。Require square root of n log n time。
 ...
 at least one of these dynamic connectivity operations must take log into。Can。So the world here。Is。U。

Update and verify。Require。Omega root n log n。Time。And that implies sorry， omega of。Loin。

For dynamic connectivity tipy。Okay。😊，I'm sorry。😡，Let now I need to describe a。

Sequence of these macro operations that I claim is going to be difficult。

This difficult sequence of macro operations is based on something called reversal permutation。嗯。

Um so。The idea here is， I'm going to。Take the integers1 through square root of n or sorry。
...
And then I take the binary encoding and I flip it upside down。

 I write it in reverse order so like for three bits。0，0，1，0，1，0，0，1，1。 This is the normal。嗯。

Sequence of bits， now I reversed that。So that's zero，4，2。

613 sorry1537 that that's the bit reversal permutation for the integer 037。

Bit reversal permutations show up all over the place， in particular。

 if you have seen fast Fourier transforms before。There's this's this part of the the testphoia transforms are often described is you take the even indexed things in your array and your odd indexed things in the array。
...
So at the very beginning， you're just permuting the order of bits。
 ...
 then combine quads， then combine out tuupples and so on。Good。Um， okay。嗯。So I'm going to write。呃。

Bit rev x to be the bit reversal permutation of some column index x。Um and。

So now here is my bad sequence of operations。Or I goes from zero to square root to n。Went to。呃。

Let's see I think it's。Update。Column bit reversal I。To a random permutation。Of size squared event。

 so I'm going to point to the next column in this bit reversal order and say。

 will n squared n factorial side to die that gives me a new permutation reroute everything within that column according to that permutation。

And then I verify。Bit rev of I。The correct。For mutation。Now this is a bit weird because。嗯。

you might think， oh， how was the hour going to know？Well。

 what the algorithm is actually asking for here is。Exece the verify algorithm on this permutation。

 you're not just going to return a true， I mean， I could run this algorithm and just say through squared of n times and that seems like it's going to do the actual job。
...
And say even if somehow I magically knew what the algorithm was going to。
...
And in this case， it's a tree over the iterations of this cur algorithm。

So let me just quickly write down。啊。The permutation for one level up。Two，10，6，13。1，9，5。13，3，11，7。

Sorry， that should be 14，15。P。If I look at any subt。And look at。

I'm sort of specifically interested in。Doing you know。

 looking at the verifies on the left and the sorry the updates on the left and the verifies on the right。
...
And so the correct permutation that I want to pass to verify but I wanted to verify that it actually works。
 ...
 depends on the random permutations that were passed to the updates on the left。A so intuitively。嗯。
...
In the left side of V。But the only way for information to pass。From the past。
 ...
 the left of tree into the future is by that information being written into cells in memory。Okay。

 so those permutations。You know that happened in these updates。Have to be written in some form。

Into memory。And then read back out of memory in the right substrate。

And so those bits are being communicated between the right and the left。嗯。So。That's。嗯。Sorry， me。

So this is encoded。In。Rights by the left subt。That or later， read。By the right subary。系。U。

If the number of things in。The number of leaves in the subtre is L。
...
Hat。From。Left to V2。Right of thing。Again， is at this point still just intuition。

because I haven't really encoded what it means why exactly I need to know this it's just intuitively。

 it seems clear that I need to know this。ok。嗯。But because I need this many bits。
...
I need this many reads and writes。然秋笼。So every level I can I can count at least n sorry at least L or sorry。

 there shouldn't be an N L there that should be。Actually another square root of n because this is the total number of operations so the L's for all the nodes on a single level add up to square root to n over two。
...
So the total number of reads and writes that I need to perform。Is N log N。Okay。

 but now I've performed squared event operations， so this means。Squared of n， log n。U her。Update。

Or verify。And because each of those updates and verifies consists of square root of n。Um。Insert。

 delete and connected operations。This means that I need。Login。Per。Indll or。Connectivity query。

All right， and so。Again， this is。Its still just the main high level intuition。
...
Where instead of verifying that the permutation up to into a certain column is what I think it is。

 I'm just going to ask my data structure， hey， what is the permutation encoded by the first X columns？

Okay， so what's。呃。Do this sort of modified thing where again， I' say for I goes from zero to root n。

I'm going to update。At the column bit reversal of。Let's call that x， a random permutation。

And then I'm going to ask for the sum。Of the permutations up to the column with reversal X。

This is going to return。By J I J minus1 impose up to pi one where j is。It runs。Of X。Thanks。So。
...
This is clearly returning more information than the original sequence of operations because the original sequence of operations that I was talking about just returns true。
 ...
 true， true， true， true， true， true it's kind of an implicit proof going on in the background this is actually saying oh。

 it's this permutation the first time then it's this permutation the second time it's actually returning a whole bunch more information so it's kind of intuitive that this this process should be harder to execute。

And therefore， easier to prove a lower bound for。Which is why why we're considering this。
...
On a single node V。And think about the information that has to pass from U to the。And。So。
...
The subt at least beges， so remember this is one node in a whole tree over time。

 so I'm really interested in you know these two intervals of operations I'll refer to as L and R when I say before V。

 I mean before the first operation in the left subte beges。北。
...
In the updates。In the right subre of V。Okay。And well this is just fixed right。
 ...
 the random choices in the left sub of V， I don't know those yet。By。And I know。L intersect R where R。

Is the set of。You know， addresses。Red。In the right sub of V。And L are the addresses。Written。

In the left subre of the。And when I say I know these， that means I know both the addresses。

And I know the contents。But you know， at the point， the contents between。呃。

TheThe left and right subtes。 So at this point in the execution indicated by this dashed line。

The contents of these addresses。A are written down somehow。Okay。And now my plane is。嗯。I can answer。

The sums。In the right。Subtree correctly。If I'm given just this information。On the other hand。

 if I'm missing anything about L intersect R， then I can't。That make sense。All right。

So this ultimately is the information that I'm going to need to encode。

Because every element of L intersect R。Represents an address that whose last right。

In the left subt and whose first read in the right subtre cross over x and every read every readrite that crosses through sorry through the node V。

 every readwrite pair that crosses the node V is going to be accounted for in this set。

And so that's ultimately what I'm count here。All right。So。Well， I mean。

 answer is just you know run the algorithm。OkayAnd let's think about what happens when I run the algorithm。
...
Another possibility is that was last written inside the left subre。

But then because I'm given L intersect R， this is something that was read on the right and written on the left。

 I know the value and okay， I know what value should be in that memory address。

And the third possibility is that it was last written inside the right subt。
...
That because I wrote it during the right subre。Based on the correct computations based on earlier accesses。
 ...
 so so I know what the correct value is。 So can I can run the sum algorithm and anytime I need to access memory。

I know the correct value that I should retrieve for memory。嗯。嗯。啊。
...
I guess there's a factor two here for each of those addresses。啊。Sorry， let me try that again。I need。

2 w times the size of L R。This is theta of log n times the size of L intersect R。

That's how many bits I need。The time the good plug can transfer over this wire。

If any of those addresses is missing。Then then the first time I try to access one of those missing addresses on the right。

 I don't know what the correct value is， and so the algorithm doesn't have enough information to correctly execute。

嗯。啊。Okay， on the other hand， these bits are defined by the random permutations that happen over on the left。

And so again， the random。啊。Permutations。In left。Of the。Require。So there are。Again。

 I'm going to overuse my notation here， let me call the number of leads capital L little L number of things that that requires L times。

Omega root and login。Bits。And so if。I do all the computation correctly。

 the number of bits that need to cross through V is omega of little L times squared n。Um。Again。

 if I sum up over all nodes V at the same level of the tree。This is going to add up to omega of n。U。

And if I then sum up over the levels， I'm going to get n log in。

And so the total number and this is1 mounting each memory access at once。

 so the total number of memory access is to execute this sequence of updates and sums is going to be n log n。

So at least one of those updates or one of those sums must take time square rootd and log in。
...
So I know in particular I know the output that ver always going to be true and I need to do something a little bit more complicated to make the layer or a bound argument go through so what i'm going to do is i'm going to simulate sum using。

Square root of n factorial verifies。Let me write this in upper caps。Okay。So up here where it says。

 hey， do a sum。I'm going to say， well。We can make the same information theoretic argument if I could somehow pull the actual sum of those permutations out of the execution of the algorithm。

 but all I've actually done in my actual real algorithm is run to verify inside that for loop。
...
I'm actually going to simulate。Running the some algorithm， but verify as a subroutine。Okay。
 ...
 so I run and ask， you know， is the sum of these permutations？This particular permutation。
 ...
 I could just ask， is the sum 0， is it sum 1， I it sum 2， I it sum 3， all the way， I the sum 99。

 is the some 100。 And at some point。The verification algorithm will say， yeah， yeah， that's the sum。

And then great， I know what the sum is， so that I've now answered the sum query。

Right so this part here。Is simulating。A single sum。Now。Over on the left。

I'm need to let R be the addresses。Red。In。You know， the right side of V。
...
So the fact that I'm doing this extra verification stuff is just yeah， I don't really care。
 ...
 so this is still the same set L。Addresses written。In the left。Side of V。 But now I potentially have。

A larger set of things that are red。Okay。So the Venn diagram that you should imagine is here's L。

Here's R。And there's this larger set。Our prime。Now what the earlier argument said？

Was if I somehow had an encoding of L intersect R prime。

Then I could faithfully execute all of these operations in the right subt。

Because anytime that this algorithm here in red from the memory address。Well。
...
You know， these。These addresses here。It's like this is。Bad。Right， so something that。呃。
...
The argument you know the kind of subtle reasons falls apart。

I don't want to encode L prime intersect R because that's too big。

I want to keep you inputing as small as I can I'm lower abounding something that's bigger than what I actually care about so it turns out the idea。

Is。That I'm going to define。A sort of abstract family of what are called separators。

 has nothing to do with separators and planar graphs， but it' just it's purely combinatorial thing。

A family of。Separators。Okay， so the idea is。For any sets。A and B。There is a separator。S。

 such that a minus b is a subset of S and B minus a is a subset of the complement of S。So the the。

I want to separate。The bad addresses。If I if I all right， so。If I read from an addressing R prime。

 I know because its address isn't encoded in R because I have complete access to the addresses in R because I'm running the algorithm。

阿咪。What I don't know is。Whether that address in our prime was last written in the deep past。

Or in the right subre or in L。So by using a separator。I can distinguishton those differences。
...
And addresses in this blue part。没置。hads an oracle that would tell me given an address。
 ...
 whether it's blue or green。Then when I run this algorithm over there。

 I had kind of have a shortcut if I noticed that the loop verify inside a loop accesses a bad address。

 an address that would not be executed by the real algorithm， I can immediately conclude， hey。

 you've got the wrong permutation， just aort the verified and return false。😡。

So I don't have to run the verification algorithm all the way to the end。
...
that would be encoded in a small number of bits， small enough that it doesn't overwhelm the rest of the lower bound competition。
