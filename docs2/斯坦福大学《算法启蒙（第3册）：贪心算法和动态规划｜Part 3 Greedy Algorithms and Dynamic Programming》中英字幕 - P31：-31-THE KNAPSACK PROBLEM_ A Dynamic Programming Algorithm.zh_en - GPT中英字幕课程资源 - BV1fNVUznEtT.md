# 斯坦福大学《算法启蒙（第3册）：贪心算法和动态规划｜Part 3 Greedy Algorithms and Dynamic Programming》中英字幕 - P31：-31-THE KNAPSACK PROBLEM_ A Dynamic Programming Algorithm.zh_en - GPT中英字幕课程资源 - BV1fNVUznEtT
...
。So now that we understand the way in which optimal solutions must be composed of optimal solutions to smaller subproblems。
 ...
 we're well positioned to turn that into a recurrence and then a dynamic programming solution for the NApsAC problem。
...
The analog of this notation and the independent set problem was G sub I。

 the graph of the first I vertices。 We're now using two indices instead of one。

 because sub problemsble have two different ways in which they can get smaller。

 recallca case2 of our thought experiments。 When we look at a smaller subprom。
...
So v subi comma x， the optimal value for the current subpro。

 it's the better of the two possibilities， so we take a max。

The first possibility is just the optimal solution with the same capacity in one fewer item。

 re'reusing that solution。The second one is you commit to using item I。 you gain value V I。

And you combine that with the optimal solution using the first I -1 items in a reduced capacity of x minus the weight of the current item。
...
So that completes step one where we think about the optimal solution structure and use it to devise a recurrence。
 ...
 now in this step two， we're going to precisely nail down what exactly are the subpros we have to care about。

Well， in our maximum weighted independence set example on path graphs。

 remember the reasoning Every time we recursd， every time we needed to look up as a subproble solution。

 it was obtained by plucking vertices off of the right of the graph。 And for that reason。

 all we ever cared about were the various prefixes of the graph。

In one dimension we've got something similar going on here， whenever we look at smaller subproms。

 it's always with one fewer item we're always sort of deleting the last item before we do our lookup。

So again， we need to worry about all possible prefixes of the items。

 so some problemsblems of the first eye items for all values of I。For the Napsack problem， however。
...
Now here is where we're going to use our assumption that I told you at the beginning in our input that sizes are all integers and that the Napsack capacity is an integer。
 ...
 So at all subpro we're going to be working with integral Napssack capacities。 So therefore。

 in the worst case， what are the various subpro that might arise。

 well perhaps each choice of a residual capacity012 all the way up to the original Napsack capacity capital W。

So now we're in great shape and step two， we figured out exactly what sub problemsble we care about in step1。

 we figured out a formula by which we can solve bigger subproblems given the solutions of smaller ones。
...
So here's the pseudocode， it's again going to be very simple。In this algorithm。

 the array A that we're going to be filling in has two dimensions in contrast to the one dimension in the weighted independent set problem。

 that's because our subprom have two different indices we have to keep track both of which set of items we're allowed to use and also what capacity we need to respect。

So the two independent variables indexing sub problemsm forces us to have a 2D array that we're going to go through now in a double for loop。
...
So for a given subproblem where you're allowed to use the first eye items and the residual capacity is X。
 ...
 the two possibilities where we have case1 where we just inherit the optimal solution with one fewer item in the same capacity。
...
So the code doesn't quite make sense in the case where the weight of the current item WI is strictly bigger than the capacity X that would give us a negative array index。
 ...
 which of course is a no no， but in that case amongst friends。

 we just interpret it as ignoring the second case of the max so I'm not going to write down the extra code。
...
And one crucial point is that you know by the time we need to solve subproblem with a given eye and a given X。
 ...
 in the previous iteration of the outer for loop， we computed these solutions to the two relevant subproblems for evaluating this recurrence。
...
So after this double four loop completes sitting there waiting for us in a in the entry。
 ...
 n comma capital W is the solution that we wanted。 That's the max value solution that can use any items that it wants and that can use the entire original NapsSack capacity capital W。

 that's what we want to return。So that's it， that's the whole dynamic programming solution for the NApsS problem just to make sure this makes sense in the next quiz。
 ...
 we just count at the number of subproblem and look at how much work we do in each。

 So how many subprom are there where they're indexed by both I and X。
...
So a couple other quick notes which play out in exactly the same way as for the Max In set problem so I'm not going to discuss the details here。

 so first of all correctness， again， it's exactly the same template as in the previous dynamic programming algorithm and in our divide and conquer algorithms。
...
So this algorithm suffers from a similar drawback to the Max independent set algorithm for Pas that we looked at。
