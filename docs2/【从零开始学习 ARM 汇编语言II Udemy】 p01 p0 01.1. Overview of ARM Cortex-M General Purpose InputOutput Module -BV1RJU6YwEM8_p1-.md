# 【从零开始学习 ARM 汇编语言II Udemy】 p01 p0 01.1. Overview of ARM Cortex-M General Purpose InputOutput Module -BV1RJU6YwEM8_p1-
...
The GI O or the general purpose input output are used for interfacefacing devices such as LED。
 ...
 timer usage and universal asynchronous receiver transmitter or the U art So we're going to find on our microcontroller the same pin being configured as either a GPIO or a special purpose IO and would have to tell the microcontroller that we want to use this pin for a special purpose or we want to use it for its normal general purpose。
...
Pin A1 stands for Ping1 of port A in the same way， if you see a pin such as this one。
...
You can use even one clock cycle to access peripherals and we're going to go into the data sheet of four different armcortex microcontrollers from three different vendors and we'll see that these buses exist in all the microcontrollers。
 ...
 whether it's a microcontroller from NXP， whether from Texas instrument or whether' from ST。

 if all got， so let's take a look at the data sheet and locate these buses。
...
I'll come over here。 So let's start with the SDM 32。 I'll say SDM 32， F 4，1，1， R E， the version of。
...
They said I've got download data sheet over here so I can click here。
...
And this opens right so that there's the data sheet and there's the reference manual。
 ...
 the power consumption limits and things like the block diagram。

 you can think of the data sheets as the summary， and the reference manual as。Indeed。

 the reference manual， so the data sheet over here has 149 pages。

 the reference manual often has over1 thousand00 pages right， so I'm going to scroll down here。

We should have the block diagram down here somewhere。The block diagram of the system。Okay。

 here we go。So I'm just going to click here to zoom and over here。
...
This's the AHP bus and anything you see connected to this shows that we can connect to these peripheral using the AHP bus。
 ...
 for instance。We can connect to。For instance， GR your port A can use AHB B。

 port B can use AHP as well， port C， port D port H can use AHB。Right。

 but it's not always the same for all peripherals。This one here is。An APB bus。As you can see。

 the name indicated here APB1， we've got APB2 here。

When coding we would keep in mind that we cannot initialize。

 let's say the U or a Uat2 and try to initialize using the AHP bus because Uat hasn't got AHP bus we work with a Uat using the APB bus right So this the bus structure of the STM32 F4 and willll be revisiting the MCU like I mentioned before。
...
I'm going to go to Google。And I'm going to search。T M 4 C，1，2，3。

TH 6 pm and I'm going to say data sheet this the Tva Sea board from Texas Instrument。

 I'm going to open this。And it's open over here， so over here the data sheet has over  a thousand pages。

 but it's fine we still look what we're looking for。
...
Those without the arrow connecting to the AHB bus can use the AHB bus。
 ...
 and this ADC here， this analog comparator here。Is pointing to APB in the same way the 12 bit AD is pointing to APB。
...
APB bus we'll talk about all of these peripherals later。
...
The popular LPC 1，7，6，8 microcontroller。And yeah， thiss the embed board。Yeah。

 but we won the data sheet。You can see use a manual as well。Okay。

So open the user manual for LPC 176 x slash 5 x。So I'll search the user manual for LPC 176 okay。

 and it's open over here， this is it right。

![](img/6bafd7714eb12d4df49178befc452bdd_9.png)

In the same way， we can just do control F and P diagram here。
...
So this is what it means to be an embedded systems developer you should be able to work with any microcontroller you provided with。
 ...
 but that's not the case if you know how to use the data sheet and navigate things like the data sheetet and the user manual you be equally comfortable working with microcontrollers from different silicon manufacturers。

Right so okay we've got this， it says simplified block diagram and as you can see we've got the buses here and this over here this bus comes here and this in their system they've got a multilay AHB matrix so this bus here is an AB matrix and there's AHB to APB bridge there's a bridge here as can see all of these peripherals use the AB bus hence they are called the AB slave group1。
