# UIUC《高级数据结构｜CS 598 JGE Advanced Data Structures — Spring 2025》中英字幕 p07 -16-Feb 11： Tango⧸multisplay trees, geometry of binary search trees.zh_en -BV1kWFGzsEmN_p7-
...
If you find something that's confusing help you get unconfused。Um，h。

 and to write coherently about those papers to an inexpert audience。

 like you were a reviewer for a conference。So。The basic structure is。UmYou'll， you know， find。
...
What the main technical tools are， what techniques they use。To derive those results。U and。

Where did you get confused？If you'd never get confused。

 you chose the wrong paper one and you have to start over。Right。

 so and so this is a little bit of a balancing act because what I'm really asking you to do is find a paper that is。
...
You know。What how。WTF and and in particular， when you're describing what for paper two。

 you should explain， oh， this is how it relates to paper one and what it clarified for me about paper one and then if you're still confused it may be that you're confused about something in paper two。

 it may be that paper two didn't completely cl what was wrong with paper one and so you'll do it again。

What are the results and how do the results in paper three connect to one and two， how did they。

 you know what tools did they bring to bear to prove those results and where did you get stuck？嗯。

In the assignment itself。嗯。I have a list。Which I really tried to keep short。

I managed to keep it down to 42 papers。These are 42 papers about data structures that were all published in 2023 or later。

And I chose these out of a list of about 200。So。This is a very active。Research area。m。

And so it actually was kind of difficult to parret down even this much。Um。

This is a suggested list of starting points， not a required list of starting points。

 so it may be that that you look at these papers and go I don't really see anything that appeals to me。

 there's also a pointer。To。A list of papers that Timothy advertised for his projects。

His presentations。A year and a half ago。Most of the papers in Timothy's wealth， first of all。

 Timothy's class was in。2003， so some of a lot of the papers that I that are in my list didn't exist at that point。

 but a lot of the papers in Timothy's list date back even further。

 he aimed for like landmark results regardless of how old they are I optimized for recency。
...
I realize this doesn't add up to five， but' being fast and looser that most of the work。

 most of the writing will be about paper one。And then。

Because papers two and three are going to be related。

 there'll be less to say about what they did that's different from what paper one did。

 so for example， if paper one is about results related to one of the things I'm going to talk about today。
 ...
 but then if paper two is also about that， you don't need to reintroduce that topic。はい。嗯。Hopefully。

You should be more worried about keeping the text under five pages。Then to。To have。

 can I fill up five pages？The hard part here is not having enough to say the hard part here is organizing your thoughts so that you only need this。

 you know， a relatively brief document to summarize everything this is again the five pages is not a hard and fast limit it is a guideline。

Um， but you know， once you get up about six or seven pages above that。

 it starts to get uncomfortable。U。There is a rubric in the， in the。

In the handout that describes you know how I'm going to score these things， basically for paper one。

 I'm going to score each of those three points， what how and what the fuck separately。

And then for papers two and three， I'm just going to take those as like whole units， so in the end。

 the grading breaks this down into five parts。嗯。And this is all。Do。March 3d。This is not， again。

 like homework zero， this is not really a hard deadline。
...
I really do need the proposals in my hands on that day。Because the next day。
 ...
 I'm going to come in and present them all。Um u。To sort of walk through them and connect them and if I have anything intelligent to say about suggestions this looks great。

 but it's way too hard， this looks great but it's not all that hard you want to aim for somewhere in the middle this is great you should look at this paper that kind of thing all of your paper chase documents will eventually be in a password protected folder that everyone who's registered for the class can see。

So you can use other people's paper chases to help you develop your project proposal。Yeah。哦，也。哪的。嗯。

Look on the front page。Just go to a front page paper assignment right here。Oh。Okay， yeah I think。

Yeah。All right， sorry。The URL is messed up， put homework there。Sorry about that。嗯。So。
...
There is an infocom paper in there， so if you've done stuff with routing in the past that might be a good paper to choose I think there's at least one paper related to databases in there if you have expertise in that area that's a great paper to choose but。

There are hopefully some papers that are going to be accessible to everybody。Okay。So。I want to start。
...
Today， by， by。Repeating the last five minutes of Thursday's lecture。
 ...
 chain display trees。Skip Sp trees。They're all variants of tango trees that have the same asmppototic behavior。

 but have other kinds of advantages。And so the the main result here is that。嗯。

These tango trees and their descendants are all log， log and competitive。
...
So organize itself to make future searches cheaper because it knows what those future searches are tangano trees。

 obviously this is a real data structure it can't see into the future because that's not a thing。
...
Although if you do your bookkeeping correctly， you don't really need to have an explicit data structure representing P。
 ...
 it's just easier to explain if I imagine that it is a separate explicit data structure。Right。

 so this is。You know， perfectly balanced。Um all。Leaves are at close to the same depth。

Which is about log in。嗯。So I'm imagining here to simplify discussion， I have a fixed set of n values。

 which I'm just going to take to be the integers1 through n。

 and I'm trying to populate a binary search tree with those keys so that later I'm going to answer a sequence of accesses。

 searches for those values one through n。Okay。嗯。So each node。N P has。
...
Child。And so。Every node is going to。Point。To one of its children。And the semantics here。Our。Left。Yes。

Last。Access。To the subt。Rooted at V was in left the left subt of V or it was V itself。And it's right。

Otherwise。给。So these。Preferred child pointers decompose the vertices of the tree into preferred paths。

 each going from some node all the way down to a leaf。嗯。So this。Decomposes。P。Into。Preferred。Ps。嗯。

All right， so。The semantics here are now， again， whenever I access a node。I make the path。

From the root。Of P to that node。Preferred。嗯。Right。If。Let's say。You know， before。

Here's another one of these preferred paths。And now if I access this node。Right when。We。Access。
...
This edge going to the left。Becomes preferred because when I access myself。
 ...
 I pretend to like my left child more。嗯。U。Okay， so everybody。Semi comfortable with the semantics of。

What my favorite child means， yeah。Okay下。I'm sorry you said that again。能你讲。That's right。

 the preferred child pointers decompose P into into vertex disjoint。Pas。Right， so these are。Vertex。

Djoint。U，And， you know， some of these preferred paths might only just be like one leaf all by itself。

All， so okay， that's a path of length zero still has one node in it。

But every node isn't exactly one of these preferred paths。It's a part。Not not of the edges。
...
Which is another reason why this is called P。Besidess being perfectly balanced。
...
The original tango tree paper used red black trees， multiplay trees use splay trees。
 ...
 the other options I listed above do something slightly more obscure。But。The balanced sp。
...
So you already know how to perform these operations in sp trees and these all need to happen in log M amortized time where M is the size of the tree。

It's not going to be n anymore because there's lots of these trees floating around。Now， crucially。
...
As key's in the balance between your research tree。 So I don't know this is going to be 3，2，1。5。4our。
...
We're going to walk down the preferred path starting at the root until we don't。

At some point we're going to fall off， okay， then we're going to jump to a new preferred path and walk down it until we fall off。
...
😡，So。If。If the search path。In P。Touches K。Preferred。Pas。What does that actually mean？Well， the the。
...
So now that edge from three to two needs to not be preferred anymore。
 ...
 so we need to split。Each。Preferred path。And then if there's something hanging off to the right of three in the preferred in the reference tree。

 then I'm gluing。The preferred path 5143 to the preferred path starting at the right child of three。

8。So and we need to concot。Preferred paths。So this is going to happen K times and this is going to happen。

I don't know， K plus or minus1 times。But what does that mean in terms of the tree that is representing that path。

 So I somehow need， you know， when I'm removing this edge from my preferred path。

 that means I need to remove。Two from that search tree。So the key observation here。

Is whatever my path looks like。Let me draw this more carefully。嗯。Whatever I'm splitting off。Seey。

 I'm splitting this off here。This is。Splitting off an interval。In the sordid order of those keys。

Everything that's below that edge I cut off is all adjacent in the sortded order of the keys along that path。
...
At most， log in because the reference TP is perfectly balanced。
...
That we're representing。Has size log n， so doing a search in that tree， doing a split in that tree。
 ...
 doing a concat with two of those trees takes it most log log n， amortize time。や。对。关全是岁。

Why do we know all the notes under the we need to be connected together in the okay。
...
So at any given time， we've got an interval， whenever we branch to the right。
 ...
 remember that every binary search tree is also a recursion tree for Quicks sortt。Okay。

 so when you recurse， the left and right subties always represent。嗯。Smaller intervals。

So what I'm really saying is this entire subt here。Actually fits between。It's parent。

And the nearest ancestor on the other side。And the rest of the path is confined within that sub。好的。

By the stories。I didn't say they were connected in the binary search tree we used to store the path。
...
And then I'm going to concatenate the left and right。

 and that's going to remove everything that has a key between A and B and do a separate thing。

And all the other things into another。1 represents P properly。 The sum takes a long long。啊，对。

How about like asking funding about game the tackleco tree， Like doesn't that still take。

 don't we need to go through every single preferred plan for us。
...
To the right side of the preferred path。
...
So what really is happening is I just walked through the tango tree using the normal binary search thing。

But sort of when I switch from one preferred path tree to another， I go， hey， I switch paths。

 I need to do some concatetnations and splits。有。So like into the data structure like the actual tree is nowhere it's just these like path trees so we have like imaginary edges kind of going from one like edge to the other or like one tree to the other well they're not imaginary edges I mean it's again。
...
So theres I'm spending within each path three， I'm spending log again time。

Doing that part of the binary search。And then log log in time doing splits and concats to reorganize that path tree。

 and then continue to the next one。有。Okay so do we have any depth giantOkay， so so， wait， sorry。都。

That's not what I'm claiming right right so the the overall claim。Is that the time？Is order K， log。

 log n。Where k is the number of。Let's say non preferred。Edges。On the search path。

You can think of this as in NP or NT， it doesn't really matter。K。Now。

Part of the reason why this is interesting。Is。That。This is Wilbur's。Inter leave。Lower bound。

So I won't go back over the definition of that， but the basic in detail。

 but the basic idea is the interlea bound is， okay， for every node in some reference tree。

 which I'll take to BPP。When I'm accessing this sequence of values。

 how many times do I alternate between looking in the left subree of that node versus looking in the right subree of that node。

 that's exactly the same as how many times do I change that node's preferred child？Okay， so those ks。

Add up to exactly the Wilbur inter leaf lower bound。Which means that the time is order， log， log。

 n times opt。Because Wilbur is lower than optt and this is lower than Lo Lo Gentime's Wilbur。

So this is a log log in competitive binary search tree。ok。Now， one of the。Untu。

Ways one of the unfortunate consequences of the way tanggo trees were initially defined。

 so tango trees originally used。Red black trees。And。That meant that the time for search was also。

Log n， log， log n。So it is possible。That every edge on the search path is at not a preferred edge。

In which case I'm going to for that one search。Im going to do- I'm going to visit login different path trees。
...
For multiplay trees。So this uses。He used display trees。As the component。

This actually provably has time。login。Of course， this is amortized。
...
But if I've got a stack。Of splay trees。Now I' not multiplay trees don't play the thing all the way to the root。

 actually， no， sorry， they do because。The search target ends up in the root preferred path and therefore it's displayedplay to the root of the root's preferred path tree。

U。But the idea is。When I'm going through this search path and I'm doing displayplays near the search path。

嗯。These。Spplays。All telescope。So in particular， when I'm using the access lemma。

 I'm defining the weight of a node。To be one for the node itself。

Plus any immediate descendants that it has that are not part of thats nodes。Preferred path tree。

All right， so you get a telescoping sum。From。The啊。Splay tree。Access。问吗。Um。With。The right。Waits。
...
Plus the change in the rank in the next tree， plus the change in the rank in the next tree。
 ...
 plus's the change in the in the next tree。All those changes add up to the overall change in the rank。
...
So。The thing that I want to talk about next。Is。The thing I mentioned。The beginning。Earlier in class。

 the geometry of binaryard research trees。嗯。This is domain。Harman。Icono。Oh no，'m going to。

 I'm forgetting somebody trashshku。Harman was a PhD student。Tshku was an undergraduate。So Petrosku。

 Mihai， Perascu one。One these CRA undergraduate research awards。As when he's an undergraduate at MIT。

 John Icono told me what he wrote in his recommendation letter。Um。

It is a pleasure to write this letter strongly recommending Miha Ptrasku for promotion to the rank Phoc professor with tenure。
...
嗯。To look at this， there was another group at CMU that did something similar specifically for the purpose of driving new lower bounds for dynamic vis trees。
 ...
 but the nice thing about this particular paper is it also led to a proposal for any other online。

Dynamic binary circuitry that looks like it might be close to competitive。嗯。Okay。

 so the idea here is have。ree。research tree with keys。One through n。I have an access sequence。X。s。嗯。

Where each u。Each access is to one of these items。So this is purely in the realm of searching。

 it doesn't include insertions and deletions or the later work does extend it to that setting。UAnd。

This access sequence defines。A set of points。Where the X coordinate is the value being accessed。

And the Y coordinate is the time at which。The value is accessed。ok。So if my access sequence is。

 for example， let's say， three， one， four， five， two。Then。嗯。The resulting set of points。Would be two。

 three， four， five， be three， then one。Then， for。Then five， then two。Okay。😊，Now。
...
When I'm doing searches for these values， those are not the only nodes in the tree that the search accesses。
 ...
 that the search touches。So。We want to like， you know。诶。Any。BST defines。A larger。Seet。

Let's call that T of x mean I'm actually going to call this set of points all set x the risk of confusing people T of x。

 this is all nodes。Touched。During。The searches， so in particular。Row I。Contains。A point。4。

All no or sorry。Let's write this again， K。Ts a point for each。Noode。On the path。

To Xi in whatever the tree looks like at time on。Okay， so we're in self adjusting tree land。
...
And let's just imagine for the moment that we're not going to change the tree at all。 I mean。
...
But that's not always going to be true。Com。Now。Okay， so they observed that。
...
So he just wanted to make people say a a lot。嗯。Okay， so what does arb burally satisfied mean。

 so if I look at any two points that are in different rows and columns， they define a rectangle。😡。

Okay， so。Any。Rectangle。Defined。By two points。And be these could be black points or they could be red points。
...
It's suffices just to say on the boundary。By induction， if there's a point in the interior。
...
And there is at least one point on the two edges defining the other corner of the recangle。Now。
 ...
 so I may only need one extra point， but in fact， let's say at least one point on here。And。

At least one point on here。ok。So if I look at this set of points that I generated by accessing the static binary search tree。
...
If I look at this rectangle， that's got another point on the top。And another point on the right。
...
I should say any arorrly satisfied superet of X。Is。Is the touch sequence。

 is the touch points for some self adjusting body any research tree？Given that access sequence。
...
There is an obvious greedy algorithm for computing not necessarily minimum。
 ...
 but a reasonably small arrborarily satisfied superet of a given set X。😡。

The so called greedy this is。The greedy ass algorithm。

It says for I goes from one to however many accesses there are。Add points to row I to make。

Rs one through I are boly satisfied。Right if whatever you decide to access on row I at time I。

Creates an unsatisfied rectangle， then you add a point in one corner of that rectangle to make it satisfied。

嗯。啊。So this is a fairly natural algorithm and the conjecture。Is。That。
...
But it's a。And the theorem says that。In an offline sense。
 ...
 that is a transcript of what happens to some dynamic violence binary research tree。
...
Are bur satisfied？Superet algorithm。Leads to。Dynamic。Online。BST。With a constant overhead。Okay。So。

Because the greedy algorithm for computing this arrbly satisfied superet is online。 I I。

 I don't get to see you into the future when I'm building。喂 point七。I can translate that。Into。

An online。Self adjusting binary search tree algorithm。And I don't do it exactly， the offline。

 given this abo satisfied superset， there is an offline thing that's one to one exact。
...
😡，It's actually something。CalledG Future。And it's weird because in the geometric setting。

You're only looking in the past。But in the binary research tree setting。

Somehow magically through through the magic of this constant time overhead。
...
Which everyone brings future searches closer to the root in particular the next search close to the root and then the next search after that as close to the root as possible subject to the earlier things and so on。

 so morally you reconfigure it using a tro，Where priorities。Are the next access times？
...
Among those nodes with keys larger than the root， the one with lowest priority and so on。Okayy。
...
This this is a little inaccurate because it's possible that the next thing you're going to access isn't actually on the search path in which case you set the priority of its predecessor and successor on the search path so that it goes up。
...
To minimize the depths of nearby future searches and then。Beyond that。
 ...
 the next further in the future for researches and so on。嗯。So。Theorem。啊。3reD s is theta。3D future。

And that has as a corollary。That greedy future。Even though it looks like it's looking into the future。

But really， if I take this。This other geometric greedy algorithm and I walk through the procedure of turning it into an online algorithm。

 again it's not exactly going to be greeny future it's just going to be close。

 the analysis is exactly the same is going to be log log and competitive。And again。

 the conjecture was Ready future is actually within a linear additive term of the optimal binary search tree。

Now， since then。Okay， I'm not going to get to the proofs today。Since then。

 there have been a couple of。More recent results。So u。The one that I added to the。Oh。

To the paper chase at the very last minute。This is paper published。year and a half ago。I so。Right。

 the conjecture was that greedy future is less than。Opt plus order。Well， order N。
...
One way of interpreting this lower bound。Is that at least in the regime where we expect the opt to be the optimum sequence to only use a linear number of operations。

 degree features login competitive。That's not exactly right。
...
The other thing they showed is。啊。嗯。I think actually the right way to。Theyy this。Yeah。Minus big oven。

U。If it is competitive。It's at least the competitive ratio is at least two right so in the original paper by domainma Harmon Patrasku at all。
...
Which is really strange。It it's what you'd kind of expect。At least what I would kind of expect。

Is that， well， actually， it's completely clear， the optimal arbarly satisfied superset of a given set of points。

Is the same as the optimal arborarily satisfied set if I reflect it。Birtically。
...
Running the sequences backwards in time should have no effect。Swapping space and time。
 ...
 whatever God that means should have no effect。Paer 13 talks about that。U。But for greedy future。

 it does have an effect。 The effect is a power of two。

 The more even more interesting one is there are。There's a sequence。

 an infinite number sequences x for each x， there is sub sequenceence x prime。

Such that the time to access x prime， the smaller sequence。
...
So sometimes doing more searches makes greedy future faster。
 ...
 which is not something that should happen for opt。So this paper。

Kind of casts a little bit of doubt on the idea that greedy future might be close to optum because it definitely displays behavior that the optimum algorithm should not have。

Now， this doesn't say anything at all about playre。

which was sort of the start of going down this road we'd like to know something about how spplay trees behave if you actually look at the way this dynamic binary search tree evolves。

 it doesn't actually look that much like spplay trees sort of a sum similarity at a very high level。

 but。The original sp tree conjecture still stands。It it's still， you know。

 we believe it's within a factor within a constant term of order capital N of optimum。

 John Iyacono has told me he has proved that theorem。At least 10 times。And then each time。

 about a month later， he finds that he ignored a floor or a ceiling somewhere。
...
And so the theorem falls apart。so it's this close is only like a plus one or a floor or ceiling off。
 ...
 but that actually does walk through the proofs going back and forth between aly satisfied sets and self-adjusting binary trees and with this online aly satisfied gives you an online binary search tree。
...
And I'll have pointers to these things on the course website。Okay。We're out of time。Thanks everybody。
