# 杜克大学《Rust编程4-5（Linux命令行工具、LLMOps）｜Rust programming》中英字幕 p79 79_04_04_构建可链式调用的Rust AWS Step Functions.zh_en -BV1Hy411q7Zm_p79-
...
Okay， let's dive into how to build step functions from AWS using rust。
 ...
 process it and then put the output into another Lada function And what's really fun about it is you can drag and drop and create your own workflows almost like working with Legos。

 And you can see here this is a diagram of the workflow And then when it runs。
...
And this would be the input to the function in terms of the output。

 we would have the payload that's being sent。 The handler is where all the work is done。

 I have an asnc handler right here。 and we say let name event payload name So I take the name value from the J payload and I say listen if the name is Marco。
 ...
 It's automatically serialized for us when we send the okay and the payload body right here is again。

 just what's inside of here。 Now to deploy this or run it or whatever。 I like to use make files。

 And so what I do here is I say make release。 and all it does is run cargo Lambda build release。

We can actually go into this directory real quick and take a look at it。
...
I would have to say cargo Lambda invoke remote。Put the name of the am 80 w Lada function here and then send in the payload。
 ...
 And then I'm going to return a payload as well。 So at first I say let the name is you go to payload payload。

 and then if I look for the word polo right that was the successful response from Marco。
...
And this will be a little bit different， but not much。
 ...
 and we change that is's going to say you lose， right， So a very。
 ...
 and let's actually just make a new one。 So I'm gonna to go ahead and say create new state machine go next。

 and it's just like Legos， I just drag one。 and we can call this one， you know， rust Marco。
...
Instead of lambmbda invoke， we'll call it Ru polo。Ras polo。There we go。 And then again。

 we just go ahead and choose this and we say Ru polo。 Now。

 what's beautiful about this is that we're done。 and it gives me the the J output if I want to copy this and store it somewhere。
...
And we can go ahead and say create state machine。 Now， in order to execute it。
 ...
 it gives it a optional， globally unique I D。 And then here's where we have to chain things together。
...
Which you can see that the payload that was returned was polo。And then we can here see this one is。

 as you win。 So， so really straightforward， actually， to， to run this。 Now。

 if we want to do a failed execution， let's go ahead and do another one。 So let's go back to this。

It we'll do another and let's just leave this default here and you can see it'll actually blow up it won't work。

 So oh oh， there's a problem error message right and we can see that it failed。

 so it's very good debugging as well and this is a great way to build reproducible services with Ru and AWS and hopefully give this a try。
