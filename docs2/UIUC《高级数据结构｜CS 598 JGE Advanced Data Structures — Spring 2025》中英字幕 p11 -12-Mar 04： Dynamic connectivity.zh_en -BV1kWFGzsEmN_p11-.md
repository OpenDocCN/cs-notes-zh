# UIUC《高级数据结构｜CS 598 JGE Advanced Data Structures — Spring 2025》中英字幕 p11 -12-Mar 04： Dynamic connectivity.zh_en -BV1kWFGzsEmN_p11-
...
Thanks to everybody for。Coming， coming back。I hope the week I was gone was。Productive or relaxing。
 ...
 officially the paper Cha assignment was due yesterday。UmReistically。

I am expecting to grade those assignments over spring break。

So if you haven't had a chance to submit the paper Chase assignment yet。
...
O。So。Last time we met。We were talking about。Applications of dynamic forests。And。
...
But specifically， this is the。Dynamic。Connectivity problem。So the idea is。I want to maintain。

Some undirected graph G。Subject to the following operations。

I want to be able to insert an edge between two vertices， assuming there's not already an edge there。

I want to delete an edge， of course， assuming that that actually is an edge in the current graph。

And I want to know。If。Two vertices， U and V are in the same component of the graph。

 is there a path in the current graph from U to V？Okay， that is my entire vocabulary。

So for purposes of discussion。嗯。
...
With one edge 20 to another existing vertex。Minor modifications the data structure will allow me to do that in similar amounts of time so specifically what I'm going to get here。

Is。嗯。These two operations are going to happen in log squared amortized time。

 and this operation will happen in log end time。Which in that login could also be amortized depending on what balanced binary treated data structure I use under the hood。
...
If those underlying balanced binary trees are splay trees， then that log n is also amortized。

If they're red， black trees or something similar then it's not。Okay， so。嗯。And。This。诶。Particular。诶。

Structure is by Hol De Luxemburg and Thorpe in 2001。Home and Duuletenberg were masters students。

Miicel Thororpe was their advisor。Okay。So。The basic idea is that。I want to maintain a certificate。
...
几。I'll call this F。So this is a maximal acyclic subgraph of the underlying graph G。
 ...
 I don't need to do anything， I haven't changed any connectivity information so I don't actually need to update any of the data structures。

If I insert。An edge between two trees in the spanning forest。

 that means I'm connecting two components of G， and I actually have to do something。Similarly。

 if I delete an edge that's not in this panning forest。

 then I don't really have to update my data structure。At least not significantly， on the other hand。

 if I do delete an edge in the spanning forest。I might be disconnecting a component of G into two smaller components。

I might not。So the difficult case。Is。If we delete。An edge in F。

Then I need to look for replacement edge。In the actual graph sheet。Yeah。Yeah， all this is undirected。

 there's a bunch of results about directed graphs as well， but they're more complicated because。

As you might be thinking， spanning trees are not going to be enough。

To determine strong connectivity or directed connectivity。And。

So the original graph G does play a role here in the data structure in that when I delete a forest edge。

 I have to look through my graph G to find a potential edge to replace it。And this is。

 in fact really the the the。The operation that drives the whole thing that makes the data structure complicated。

Is how do I find a replacement edge connecting these two components of the now no longer a spanning forest？

Without doing a brute force search over the entire graph。
...
Over time。嗯呃呃。As long as the edge stays continuously in the graph。

 over time its level might decrease， but it will never increase。系。嗯。Can decrease。Over time。

But initially， all levels。Are equal to log in and again。

 as usual I'm being a bit fast and loose about floors and ceilings。

 just imagine the number of vertices is a power of two。嗯。嗯。

Now this level assignment is an artifact of the data structure。
 ...
 So let G sub I be the。Subgraph。Consisting of edges。With。Level at most I。

And the invariant is the number of vertices。In。In any component of GI。Sorry， each。Ch。
...
That's defined by taking the global spanning forest and just restricting to edges with level at most I Okay so in particular。

 F of log n is just equal to F。And F of zero。Is the empty forest that has all the vertices。

 but it doesn't have any edge at all。Each component in F0 has exactly vertex。So this。

You can kind of think of this。As。诶。You know， hierarchical。Am。Clustering。Of the spanning forest。

That you imagine。You can think of it top down as well I've got this big spanning forest and then to get to level log n minus1。

 I'm going to remove some constant number of edges and now I've split my forest into a few components that are smaller might that each contain at most half of the vertices。

And the next level down， I'll separate into smaller components。

 each of which has at most a quarter of the vertices and so on all the way down to the ground at F0 where there are no edges at all。

 again， initially all edges have level log n， so initially F is F log n is just a spanning forest and all of the lower level forests are empty。

But over time， because of maintenance of the algorithm， I'm going to be。

Reducing the levels of edges and thereby growing these smaller forests。嗯。嗯。We to。Maintain。

Each component。Of。Each forest FI。In。An oiler to tree。
...
I'm not ever going to need to do path operations。It also turns out that if you happen to know that the graph is planar and you're doing insertions and deletions that are consistent with keeping it planar。
 ...
 you can use。Again， this sort of tree co tree idea to speed some of these operations up by using a pathbased dynamic forest data structure in the dual graph。

 but I'm not going to talk about that here just there is an oiler tour tree for every component of the spanning forest at every level。
...
So I need some constant amount of information for every edge， in particular。
 ...
 I need to know what level it is。And then because。A tree within vertices has at most n minus1 edges。

 number of the total complexity of the spanning force is only order n。
...
And each edge that's in the spanning forest is represented in a most log n of these data structures。
 ...
 but this is not where the interesting part is。嗯。嗯。Now。There is。

One additional piece of information that I need。So now remember that the Ouler to trees。啊。Am。

Each oiluler tor tree maintains。An unrooted， undirected tree。Subject to these three operations。Um。

Li U andV where you en you're in different trees to merge two trees into one。
...
So I don't remember whether we talked about the fine tree or fine root。Operation in much detail。
 ...
 an oiluler tour tree is a splay tree over an euler tour of the tree that's being represented。

 a pine tree。Basically just walks up the tree to the root and reports the address of the route。

So in particular， you can also think of this as。呃。Asking。Whether。Two vertices。

Are connected in the forest。So that connected because it's an oiler tor operation。
...
And I'm never going to ask whether vertex U at level5 is in the same tree as vertex V at level 17。

I'm only going to do these queries within one level of the data structure。嗯。U。

But the other thing that I need。Has to do with these level things。

I do want to talk a little bit about the structure of this spanning forest FI。U。啊。S。Right。So。
...
So if I can connect things down at level I， then I will。嗯。So。Yeah。

 so I need to make sure that in particular， this is a spanning forest。Of G sub I。

 that's another that's part of the part of the invariance that I want so there there's really two invariants。

One is the structure that the forest actually still is spanning force the components。

 the second is that the components aren't too big。Right。

 and these the first invariant implies that F is a minimum spanning forest of GI with respect to the levels。
...
On the other hand， edges that are inside any component could have any level at all。Now。
 ...
 this is an edge in G， the original graph。Right。嗯。So let me say this。For any edge。Uv N G。

If U and V are indifferent。Components。Of G sub I， then。Level of UV must be greater than I。

But I don't know this within a particular level。So but what's going to happen when I decide to remove an edge。

 say here's my picture of one of these trees。 and I decide I'm going to remove this edge。
...
So every。Vertex。In G sub I。Nos。Let's say the number of incident。Edges。at。Level I and in particular。
...
Is I'm going look at the smaller of the two components that I get when I remove that edge from the tree。
...
Within each of the within each。啊。For each vertex in that smaller component， I'm going to ask， hey。
 ...
 I can reconnect。ButThat search is essentially going to be brute force。

But if I really made it brute for as in I just look at the vertex and ask， hey。

 ask all your edges if their level I and connect to the other tree， that would just take too long。

 So I'm using the euler tour tree。I I need to augment the oiler to retreat to allow me。
...
I can use the Euler tour tree。To。Enumerate。All。Non tree。Edges。嗯。With。Level I incident。To a component。
...
Um。Subtes of the Ouler to tree correspond to intervals。In。In that Ouler tour。
...
In order traversal。Of the oiluler to tree。But every time I get to a node and I want to know。

 do I need to continue the traversal further down into the node， I'll first ask， hey。

 are any of the nodes that are represented in this part of the Ooratory tree incident to level eye edges？

😡，Now， each individual vertex knows this， knows individual vertices correspond to individual nodes in the Ouleratory tree。
 ...
 then I will continue the traversal in that subtree and if that interesting bit is not set。
 ...
 In fact， this is even conservative。 if the number of edges is like polynomial in N。

 you can get rid of the log factor。Rather than。Having to diverse the entire earlierterary。Okay。
...
Probably the most technical part of the data structure。
 ...
 now would'll be good time。Okay。So。Let's。Think about how this works。Okay， so if I want to say。Um。

Connected UV。The way I would implement this is。I would ask this is in if U and V are in the same tree in the top level forest。

A， so this is one。Query to one Euler tree data structure， so this is only going to take log end time。

Amortized if under the hood， this is a sp tree， and I should also say that that K log n is amortized if under the hood display sp tree。

Okay， so that's。Sort of the easy part。嗯。Insert an edge between U and V。This is a link。U the again。

 in the top level a forest。U。And notice when I do this， I might need to。Update。Level。Edge。Counts。

In F logN as well。So this is what I'm referring it to here。levelvel。Edge。Counts。But again。

 in the if I just treat this insertion in isolation。啊。This also gives me。Log in time。But。
...
Pay for those changes in level now。So in particular， anytime I change the。Level of an edge。
...
I need to update the level edge count stuff at the two endpoints of that edge if I'm reducing this level from I to I minus1。
 ...
 then in level I these two endpoints now have one less level I edge incident to them and at level I minus1 these two vertices now have one more level I minus1 edge connected to them。
...
eitherither incrementmenting or decrementing them， that's going to require doing。
 ...
 you updating some of the summary information in the balanced binary trees。

That's going to take log in time。嗯。So。Log square and amortize time。 But now that I've paid for that。

In the deletion algorithm。Decreasing the level of an edge is now free。嗯。Okay。So。This is them the。

Delete U。This is the interesting version of the more interesting operation。Right so。

If UV is not in the top level forest。Well， you're done。

You don't really need to do anything technically， yeah。

 you should probably there's some level count stuff when you delete but。mThat's going to be cheap。

 it's going to only be log ribbon time。That's not the interesting part。系。So otherwise。U。诶。For all my。

Greater than or equal to the level of UV。I'm going to cut UV out of the forest at level I。系。😊，Again。

 this is going to require updating this other interesting information。

And now I need to look for a placement edge。Okay。嗯。So。Because。The top level a forest， and in fact。

 every level of forest is a minimum spanning forest with respect to levels。

If there is an edge that reconnects。嗯。Two components of the top level forest F log n。

The level of that edge must be at least I。So this replacement medge。Which must。Have。Level。
...
At least I can't have level less than I。Because if it did。
 ...
 because that's how minimum spanning forest work。 Remember the defining property of minimum spanning。

 one of the defining properties of minimum spanning trees。Is that if I look at any separation。

 any partition of the vertices， the minimum weight edge that crosses that partition must be in the minimum fanning tree。

So if there were a lower level edge crossing between the two components of the forest that I just separated by deleting that edge。

That would have already been in the minimum spanning forest。Okay。嗯。So。

The way that I'm going to look for these edges starting at level I and working my way up。嗯。So can。

F I goes from。A levelvel of UV to log in。If I can find a replacement edge with level I。
...
Now this is a bit of an ambiguous notation because T is different on level9 than it is on level 10。

I should really write T sub I comma U， T sub I comma V。

 but these these I think the extra notation is actually going to be more confusing。
...
Of T is less than or equal to the number of vertices of TV。So if T U happens to have more vertices。
 ...
 just swap the names U and V。And swap the variable names this again is an independent choice made at every level。

啊。Okay。We need to scan。UTo you。4。Incident。Edges。With level equal to I。Would the other end。Is in TV。

So I'm going to look for a replacement edge exactly at this level。

Using the stuff that I said earlier， so I will scan through all of the edges that have the right level。

Incidents to this tree T。That have， that have and and in check。

 do any of these have their other end in TV。嗯。嗯。If we find。1。Um。I know let's。Yeah， let's。As we find。

Let's call it X Y。Where x is in T U， y is in TVv and level of X Y is equal to I， then we link。X to Y。

In Forest Ie。Is that right？UActually。Also For I+ one all the way up to For login。Okay。

So we find that if we find replacement a replacement edge at level I。

 that is also going to be a replacement edge in the next coarser forest F minus is FI+1。

 the next coarser forest F+2 all the way up to the very top， the global spanning For F。诶。Now。嗯。

If we find。Um。4。Every other interesting edge。X Y。I'm now going to change the level of XY。

Two I minus1。Okay， so the scan TU for incident edges at level。

 so maybe I should use my own rules here。And just say。嗯。For all。Edges。X， Y。

 where x is in to U and the level of。The level is equal to I。If Y is in TV。
...
Okay。嗯。So。Notice one thing that happens here。Is。I need log N time for each of these edges X Y。😡。

That was this stuff that I wrote up here in green。嗯。But。One of two things happens。Either。

I need to do a bunch of link operations。 So those I have to pay for my explicitly。

 or I decrease the level of the edge。And decreasing the level of the edge has some overhead to it。

 but I've already paid for that overhead。In particular。

 decreasing the level of an edge costs me about log n time doing the various link country stuff to update those weird level counts。
...
Add in here。Which is。Basically， for all。Edges。In TU set the level of that edge to I minus1。Now。
...
why I only need log N levels， but you'll notice here this tree in level FI that I've split in a TU and TV。

 that is a spanning tree of one of the components of GI。

 and so that component has at most two to the I vertices。

 and I split that component into two subcomponents。

And I changed the level of the smaller of those two subcompons。So TU has at most。😡。

Two to the I minus-1。Vertices。So this is I'm maintaining this invariant that the component sizes are bounded by twice sorry to the level right so this condition guarantees that the number of vertices。
...
The size invari guarantees that I guarantee that I only need to go down log n levels before I'm left with no edges at all。
...
Is log N。And each of those links。Takes me。Actually， this isn't really even amortized。
 ...
 the takes speed log in time because I'm doing a constant number of operations in an oil or tree。
...
嗯。So in the end。I can do。I need log squared n amortized time。Per insertion or deletion。
...
Building data structures are they thinking about the way data structures are designed and analyzed。

Very much feels to me like a house of cards。Everything has to sort of there's lots of different ways that you can put things together。
...
So there's a lot of different pieces happening here that are affecting each other。 It's like well。

 there's this invariant， but how do I how do I know that's maintained that implies there are this many levels。

 I've amortized this over there because I did that to the data structure。

 but I had to add this extra information So there's there's a lot of pieces floating around。

And so I would expect。If you're like me。It's going to take a bit of time， to sort of。

Get a comfortable enough feeling with no， I can't hold all of the details in my head。But I kind of。

 if I glance pay attention to this this component， this connection seems to work and this connection seems to work and I sort of get a feel for I kind of see how it might possibly be plausible。
...
I'm not expecting it to go now go implement it， it's not like that。So again， please。
 ...
 if there are parts of the description that was like， I didn't get that， please point those out。Okay。

嗯。So this。As I said， was by。Halold Di Kberg and Thorpe in 2001。嗯。
...
It's maybe the number of vertices with the bookkeeping， but linear time in the worst case。
 ...
 so if you're trying to do anything real time， this just sucks。Um so。What if we want， you know？

What if we want worst case behavior？Okay， so there's actually an older data structure by Frederickson。

thinkIt's 87 ish。That gets square squared of M time。Squa root of the number of。嗯。Vertices。

 that was 85。呃。That was the best known for about。10 years。And then David Epstein and his。

As a PhD student and his advisors came up with a way of so- calledled sparsifying the graph。
...
So that gets it down to square root to N and that was the best known for another 20 years。

 the current best。Is by Julia Czzoi and her students from 2020。呃。

I think actually the right way of saying this is。It's subponomial。

That is how they report the running done。It's less than end of the epsilon for any epsilon。

But it's probably super， super polyaugorithmic。嗯。There's a song that somebody published。

In Siggac New that was sung to the tune of the Mary Poppin song， Superup caliraagiclyvic XBL Laosius。
...
But if the algorithm says no these vertices aren't connected。
 ...
 there is some chance that is a false negative。That chance is exponent is n to the minus C where C is one of the constants inside the big O。
...
Except through the answers to the connectivity queries。The data structure that I just described。

Handles arbitrary insertions and deletions， which means even if the adversary can peer into your data structure and go。

 ah so that's how your forest and how your levels are organized。

 I'm going to choose this edge just to screw with you。

 you still get the same amortized time balances。Um。This is a serious balancing act here。

 there's actually a vague description of this algorithm in Wikipedia of all places。This is 2013。And。

Then let's see。Wollf Nielsen。In also， I think this was 2013 got this something that was。

Long squared n over log log n where this was。Las Vegas。And public。嗯。No。Sorry。There was。

A different algorithm that ran in time into the little of one。That was Las Vegas in public。

 and Chiszoy derandomized that algorithm。So I'm not going to even attempt to explain Chizoi's algorithm because it's based on removing randomization from an already complicated randomized algorithm。
...
UAnd。This is by。Hong et all。2023。This is a nice 56 page paper。

It actually includes a fairly thorough survey of the past results。

 unfortunately I did not find this paper in time to put it into the paper chase document because it's based on a couple of different conference papers that are several years earlier and this particular journal happens to be about three years old。

 so it's not really on my radar yet。U。But。Let's just say。对。You know， this is complicated。

There is really still a fairly significant open question here。Is。Right。

 so I can get that expert you know， expected amortized， I get log squared and worst case amortized。
...
U possiblesible。This is open。😔，This is not a。Problem that I propose anyone work on for their project。
...
😡，There are at least。A dozen people who were smarter than me。Epstein was my master's advisor。
 ...
 Frederickson was at Purdue for many years， Julia Chishoy is at the Toyt Institute of Chicago。

 I wrote there was an intermediate result by Christian Wolf Nielsen whose brilliant graph algorithms person out of Copenhagen。
...
嗯。The last thing I want to say is a lot of the techniques that I've talked about here and that go into these other papers also go into other kinds of dynamic graph connectivity questions so you could reasonably ask。
 ...
 you can ask about。Minimum spanning trees。So I've got a minimum spanning tree of a graph and I want to maintain it as I insert edges。
...
So you can ask bottleneck path queries between two vertices as your data structure access to the minimum spanning tree。
 ...
 a graph is three connected if between any two vertices， there are three vertex disjoin cos。
...
Does anybody have any questions about this stuff？Yeah。The biggest not。
...
So when I say log n over log log n squared， that's the expected running time similar to like using a tree or randomized quick sort。
...
But the user is not allowed to know what that spanning forest is。
 ...
 and so the update can't depend in any way on the data structure's decisions。
...
I public the public for climate director is not just like an im。

 would you ever like go into a problem with like， hey I'll let it go？差で。So the。
...
It's kind of along the same lines as。When you're using a balanced binary search tree。

YouIt might be easier to get something that guarantees logithic time。
...
I'm struggling to come up with a concrete example here。好。Oh。Pro log access。you know yeah， kind of。

 I mean， one of the things that subtle about the Karov thing is that there are multiple sources。
...
So it's not at all clear to me whether this privacy assumption is something that affects correctness。
 ...
 affects actual speed， or only affects the analysis。It。Complicate。Did you have another question。Okay。

All right， thanks everybody， we'll see you on Thursday。
