...
本节课中我们一起学习了Python的变量与数据类型。我们知道了变量是存储数据的容器，并掌握了三种基本数据类型：**整数**、**浮点数**和**字符串**。我们还学会了如何使用 `type()` 函数查看类型，以及如何使用 `int()`、`float()`、`str()` 函数在类型间进行转换。牢记，为变量选择合适的数据类型，是写出正确、高效程序的关键第一步。