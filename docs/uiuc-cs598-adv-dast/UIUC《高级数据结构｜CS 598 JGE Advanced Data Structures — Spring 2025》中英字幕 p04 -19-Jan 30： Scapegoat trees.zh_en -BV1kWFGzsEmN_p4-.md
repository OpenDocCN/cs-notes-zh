# UIUC《高级数据结构｜CS 598 JGE Advanced Data Structures — Spring 2025》中英字幕 p04 -19-Jan 30： Scapegoat trees.zh_en -BV1kWFGzsEmN_p4-

Okay。

![](img/7d942806f207614d2fe32ecde3453675_1.png)

So。Thanks for coming out through the drizzle。Of。What I want to talk about today。
...
It's the idea of a balanced binary search tree， so we all know what binary search trees are。
 ...
 you have a collection of nodes， every node has a search key in it and has a pointer to a left subte and a right subte。

And in order to be a binary search tree。Sorry， in order to be binary search tree。

 all the keys in the left sub tree have to be smaller than the key at the root。

 which has to be smaller than all the keys in the right subre。

 and the subtes have to recursively be binary search trees。嗯。

So an inward reersal gives you a sorted list。Balance typically means。That the depth。Of the tree is。
...
I to be a little bit vague about what I actually mean by search。
...
Is this item in the tree or find me the predecessor of this item in the tree or find me the number of items between these two values。
 ...
 find me the largest item between these two values。

The different lots of different searches all ultimately boil down to traversing。Either one or two。
...
225 talks about what I think is really the first。Balanced binary search tree that was ever proposed。

By Adelson Velski and Landis in 1967 so called AVL trees。
...
If I take bead to be a constant， these are not technically binary search trees， and for that matter。

 neither two，3 or two， three，4 trees， those are actually examples of bee trees where the degree of a node can vary within some range。

But you have an additional constraint that all the leads are at the same depth。And so instead of。

These guys。Are usually。Handled by using rotations。Exactly the same operation that you saw in the homework。

Um。The you know， one， two trees， brother trees， bee trees， et cetera， the these are handled by。U。

Taking large nodes and splitting them into smaller nodes or taking pairs of small nodes and merging them to larger nodes。

All in order to maintain this log Earthic depth。开幕。There are。Dozens of other variants。The 1970s。

Computer science literature。Is sprawling with papers about balanced binary search trees。

 dozens and dozens and dozens of different variants。嗯。😊，Generally。

 we stick to either AVL or red black。Partly because the algorithms are simple because they've gone through the ringer of being studied by dozens of people。

 they've been simplified and in the case of red blackt because one of the authors of the red Black T Pa is an evangelist and so he got red Black trees put into the C++ standard template library and then 10 years later he fixed it。
...
Balanced binary a search tree that uses randomization to stay balanced。
...
So what I really want to focus on for the next couple of lectures is just。

 I just want the amortized time to be good and it's also I'm in the regime where I don't care about the performance of individual operations so much as I care about the aggregate performance of the data structure over its whole lifetime。
...
It's a particularly simple data structure because it does not rely on rotations to the rebuilding。
 ...
 it relies on a black box that says， hey， here's some data，Build me a perfectly balanced binary tree。

 and that's it。So in some ways， scapegoat trees。Use the。You know。Users。Sort of either。

Global or local。Rebuilding。Similar to the Bentley sacks over Mars van Lon stuff that we saw yesterday。
...
But I do have guarantees that the amortized time to do any operation I can think of is log N。
 ...
 and if a research needs to choose one of them， I need to make end decisions。
...
But sometimes you know something about the query。Or you know something about the insertion of deletion that allows you to sort of bypass that information theoreticaloretic lower bound slightly and so splay trees have lots of magical properties that we're going to continue using later in the semester。
 ...
 but I want to start with something simpler。Skip goats。嗯。All right。So I'm going to start。
...
I only care about doing。嗯。Searchches。And。Delicions。诶。😊，So if you。It's a perfectly balanced tree。

With a loose。启工。Connecting it to the screen so automatically。



![](img/7d942806f207614d2fe32ecde3453675_5.png)

We know that search is going to take log of capital at time， this is a worst case time bound。

 the question is what to do with delete。Okay。UNow there's a couple of different ways to do deletions if you think back to your undergraduate data structures class or into programmingm class。
...
Say it doesn't matter which one。Then I'll just I'll redirect。The啊。

Pointer to V to now point to that child W instead。And3 W。啊。So this is the node that I'm deleting。

And then finally。If V has two children。I'll look for its successor。

Which is always going to be the rightmost node in the left subt。I'll。Sp。These two。So now this is a W。

This is a search tree that's a search tree and now V is sitting here now at this point it's no longer a binary search tree because the nodes V and W are out of order。
...
I don't care。It's a black box。 it does some stop if I asking questions。
...
That's not the problem。不能是。I'm sorry， well， but that's fine， that's what we were told to do。
 ...
 we're told to only support in searches and deletions。😡，Yeah。我上。好。

Maybe it takes long could just Well， I mean， so another way to do this for some kinds of if I'm only doing membership queries is I could just point at the node that I wanted to delete and say it's gone。

And modified the search algorithm to when it reaches a tombstone is the target of its search to report false instead of true。
...
😡，I。Okay的。And again， that will work for membership queries， but it won't work， for example。
...
There's no bound at all。Okay。So we need to。Do something to fix that and this is where all the balance binary research tree stuff comes from。

 right？So I'm not going to do rotations， I'm not going to do node splits。
...
Rebuild the tree。And when I say rebuild the tree， I mean。
 ...
 or if I'm using tombstones， only the nodes that are real。Not the tombstoneun notes。

Or if I'm using a data structure and an anti data structure， only nodes that are in in。

In the real data structure and not in the anti data structure。系。So。

What this guarantees now is now I can do the analysis in terms of capital N。

Where I say this is the size at， let's say just after。The last rebuild。The most。Recent。Rebuild。
...
So log of Big N is also。Dig O of。Log little in， this is for search。Okay。So great， a restored。

 worst case， efficient search time。According to the value that I want。

 log of the number of items in the data structure。And in fact。

 I've restored it for most deletions as long as I don't need to do a global rebuild。

A deletion also takes log of little end time。But every so often， I have to rebuild the entire tree。

 Now， rebuilding the entire tree I can do in linear time。

One way to do that is just do it in order traversal。

 transcribe it into a sortrded array and then rebuild the tree from the sorted array。

 another way to do it is using a linear number of rotations so don't actually need to allocate any new memory。

 like an array， this is one of the things that informs the rotation and swap question in the homework。

So， but you know， worst case delete time。Is you know。

 order end time and that's that that really is little end not the。

A big of a little in big of a big under are the same thing。But we only care about。U。
...
So the number of deletions I've performed since the last three is at least the number of items that are still in the tree。

so what I can do is。嗯。Charge。Order one time。For rebuilding。To each。DeletionDete operators。俾。

And if I actually， I should make that thehao of one。

I charge some constant amount of time to each rebuild or sorry to each deletion。
...
That even when I need to do rebuilds。The time for deletion is now log and amortized or actually a more precise way of saying this is。

Log in worst case time to find the node to delete， plus order one amortize time to actually do the deletion。

😡，诶。Great。So。Now。Searchches plus。Inserttionions。Now， in terms of writing code。

If you've played with these data structure things and balanced byvent trees at all。

 normally deletions are more complicated to get right。Then insertions。

This makes deletions completely straightforward， you just need that black box that says。

 here's a binary search tree， give me an equivalent thing that's perfectly balanced。

That's all you need。For scapeboardat trees， this is flipped。

And the reason this is flipped is I actually need a lot of deletions。
...
I haven't really I can't have affected the depth， it's still log of n where n is the current number of items。
...
Is。When。A sub tree。Becomes。Sly imbalanced。Rebuild that subre。はい。

Now that subre might be the whole tree。But the idea is。Small subrees far away from the root。
...
If you only have three nodes， there's a balanced tree and there's an unbalanced tree。

 it's really easy with one insertion to make that unbalanced。

 so small subtrees are going to be rebuilt more frequently than large subtes。And this really。

 in some sense， comes down to a question of scheduling。Okay so small subtes。Are rebuilt。More often。
...
But it only takes me five steps to rebuild that subterary。So on average。

 that subre is only going to cost me constant time per insertion I'm going to rebuild this subtree with a million nodes in it。

 every one million insertions。That means on average。

 I'm spending constant time maintaining this subtery。RightAnd so the intuition is that。Essentially。

 when I do an insertion。I pay a constant tax for every node along the insertion path。

To pay for the future rebuilding of the subary rooted at that note。

And so the overall tax that I pay is only going to be logarithmic because the tree is guaranteed to have log in depth。

And so my amortized insertion time is login that that's the intuition。

 so maybe I'll write this down so intuition。啊。The insert algorithm。Pas。A constant tax。At every node。

Z on the search path。To pay for。Future。Rebuilds。Of the subre。Rooted at the。Okay。So one question is。
...
Okay， so。A。The first。Version of this。U。This is something called weight balanced trees。Okay。
...
And eventually that's what we're going to do so basing your your rebuilding strategy on the depth of the height of the tree is called the height balanced tree AVL trees are height balanced red black trees are implicitly height balanced bee trees are height balanced I'm going to use weight balanced trees right so these are also sometimes。

Seen called in the literature BB Alpha T， Boed B is what BB stands for。

 and alpha is a parameter of the tree。So say that a node V is alpha balanced。If and only us。

The size of the left subre of V。Is at least alpha times the size V。
...
This is at least n over four nodes that the direction that I didn't take starting at the root。
...
And it's not exactly going to be log alpha then because alpha is less than one and this is not right。

I think it should be something like。啊。Log of one minus1 over alpha， and I think that's right。

So every time I go from a parent to a child， the size of the subt decreases by at least a factor of1 minus1 over。

 I don't trust my arithmetic here， verify that there's some simple expression。So if alpha is。嗯。Oh。

 sorry。There's a one over there。Yes。Let me write it differently。Well， I'll just leave it alone。 Okay。

 I I think this is correct。 If alpha is one half， then that expression reduce it down to just two。

One over one minus a half， it's one over a half， it's two。 if alpha gets smaller。

 that expression gets bigger。So the more imbalance that I allow in my tree， the deeper the tree gets。
...
I'm going to walk back up the tree and when I encounter a node that doesn't satisfy this condition。
 ...
 I'm going to rebuild that subre。That's my rebuilding trigger。And so why does that work？so。

And I do an insertion。Inst a new node V。Walk。From。V to the root。啊。If we find。An unbalanced。Noode。You。
...
After we've rebuilt you or any of its ancestors。U is perfectly balanced。
 ...
 right the size of the right sub of U and size of the left sub of U are exactly equal。

Every time I do an insertion into the subt rooted at you。

Either the left sub treee or the right subree goes up by one。

 and the total subte size goes up by one。But the ratio between the subre sizes and the whole size on one side goes up a little and on the other side goes down a little。

In order for the tree to be unbalanced， say the left subtree to be twice as big as the right subtre。

I had to do at least as many insertions into the left subte as the size of the right subte。

But the size of the R of tree is a third of the size of the whole tree。
...
系。So。They's just say math。I rebuild。Subre。Of size。Okay， only after。Some constant times k insertions。

Into。That。Subtracte。And so that implies。嗯。That for every node on the search path。

I those are the roots of the subtre。 When I insert something。

 I'm not just inserting it into the whole tree。I'm also inserting it， for example。

 into the right subt of the root and into the left subtre of the right subt of the root。
...
That's the what we're looking。Now， how do you detect when the thing is unbalanced？So。

The easiest thing to do is。Store。Size the。At V。So every node will keep track of in addition to its search key and its left and right children。

 it'll keep track of the number of nodes in its sub。When I do an insertion， I increment that counter。

 and then testing whether or not is balanced or not is just two comparisons。

 a little bit of arithmetic。So a little bit more overhead in the space。
...
But then you have to do something different。Um，Uh， which is where the word the the phrasescape cut tree comes from。

 but before I go on that， let me just make sure that that everybody's on board。With how this works。

Okay。Great。So now height。Balanced trees。The depth。Of。Well， actually， let me， let me。
...
I think the alpha is like 1。44。诶。Um but。As long as。Alpha is strictly larger than one。
...
The strategy that I'm going to show you will will work you'll get trade offs larger values of larger values of alpha means you're allowing the tree to be deeper and that's necessarily going to increase the search time。
 ...
 but increase make updates more frequent。Okay。So the re strategy is。When。The height of V。

 when I notice that the height of v is larger than log base2 of the size of V。Rebuild。UhThe sub。
...
And I could do this by storing the height of every node。In as a new field of that node。
 ...
 but it's just doing the right accounting that's only going to take it you know it's either going to save the same or it's going to increase by one depending on how many steps you took to get from V down to the new node that you're inserting so again this is a constant amount of overhead。

😡，But this is not actually the。the strategy that scapegoat trees use， right？Soscapego trees。

Just say when the depth is bigger than alpha times log n。There's a parameter alpha。

As soon as the whole tree is clearly unbalanced。嗯。Walk。Up the search path。To。The first node。V。

 so this is the deepest node on the search path。Such that height。
...
Right， right， so you first insert using the textbook algorithm and you do a search。

 it's unsuccessful， you replace the last no pointer with a pointer to a new node containing the。啊。

你 certain啲到低。So I'm deliberately omitting some floors and ceilings in plus older ones。So in fact。

 any subree that has only log n nodes in it。You can just leave it alone。

Because whatever you're doing in that sub of size log n takes order log n time so whatever。

So you really only have to trigger this when the subries get big enough to actually matter。

 another way of saying that is just you ignore the bottom three levels or something like that。いや。

跟不包同的。Ah， but this is the trick for scapegoat trees， you don't need to rebuild multiple nodes。
...
I'm actually sort of inserting into this nested sequence of subtes。Right， and say。You know。
 ...
 this subre is bad and this subte is bad and this subtree is bad and then this subt is good。



![](img/7d942806f207614d2fe32ecde3453675_7.png)

Now。One， just before the insertion， everybody was good。By inserting that one node。

I made some of these subrees bad and I don't know， maybe even know the subte up here is bad， right？
...
Unbalanced。So。Rebuilding。Any。Proper subtre。Any bad proper subt rebalances it。So in fact。

 I only ever need to rebuild one tree。So this is the idea behind us a scapegoat is。

But the tradition is that at。I think this is something that the Vikings did， certain times of year。
 ...
 we're going to use you。And then they drive that goat into the desert or off a cliff further into the ocean。

 or at least out of the village forever。And that cleanses the community of the sin。

That was committed。 So you blame the scapegoat for the tree becoming unbalanced and you punish it by rebuilding。

系。And because， again， the same idea behind the scheduling that I did earlier based on weight。

Weight balancing again， applies here。You can argue that。If。V is unbalanced。嗯。

We've seen omega of the size。Of the insertions。Sinceense。The last reboot。

This takes a few lines of algebra， it'll be in the lecture notes。

 but essentially you argue that if the height is unbalanced。

 then in fact the weight is unbalanced as well。And then you just reduce the weight argument， yeah。

Are ways or heights。I'll get there， so I'm still approaching the actual final strategy。Okay。So。

The final strategy。Is I use height。To tell me， hey， I need to rebuild something。

 but I use weight to decide what to rebuild。Yeah， yeah， this is the last rebuild at V。

 the last rebuild of the sub rooted at V。And when I say insertions。

 I mean insertions into the subree rooted。This is all local information。嗯。So。

When we discover that the depth is bigger than alpha times the log of the size。We walk。Up the tree。

呃呃。First。Rebud。To the first node。Such that。嗯。Well， the first node that is。That is。Wait。Balanced。

This is the actual Sgot tree。And the reason I know I can do this， first of all。
...
And the reason is that if the tree is too deep。Then for the right value， I mean。
 ...
 there's parameters here， so's I made a mistake by using alpha to mean two different things。

 but if the tree has depth more than alpha log n， then some node is beta unbalanced。

 where beta is a simple function of alpha。😡，Um。So there there。Will。B。A scapegoat。All right， so。

When I insert a node and it makes the three too deep。
...
So I started the new node V and I compute the， what's a leaf， so it size is one。Its that balanced。

 Yeah，0 and 0。 It's fine。 Now I look at the parent of V， and I compute the size of that subre。

Is it balanced right， Okay， then rebuild Otherwise I look at the grandparent of E and I compute the size of that subte。

 most of that I've already done， I just need to do the other half。

And so what I do is I walk my way up。At every node。I compute the size。
...
Compute。All sizes。Below the scapegoat。Is order of the size？Of the scapegoat。

But I'm already going to spend time proportional to the size of the scapegoat sub to rebuild it。

So this is a general design trick for all kinds of algorithms and data structure problems if I already know that I'm going to be spending log end time on something doing anything else that takes me log end time is free。

If I already know that I'm going to be spending linear time on something。

 then anything else that takes on a linear time is free。

So I already know that I'm know you spend time proportional to the size of the scapegoat subt。

 rebuilding that subt。So the time that I spent finding the scapegoat tree。

The finding that scapegoat node is free。It's just， I just charge it to the rebuild plan。Yeah。

total size of the treatment。Okay， so I literally am storing three integers in addition to a raw off the shelf textbook binary your search tree。

 I'm storing the size of the tree， I'm storing the depth of the tree。

 in fact I'm not even sure I need to store the depth。U。Sorry。

 I need to check my notes what exactly they are。嗯。The size of the tree， the height of the tree。

 and when we start doing deletions I need to record the number of deletions。看。

And I'm not even sure you need the height of the tree。

Because you compute the height of the node when you bridge it。
...
Login insertions。This is amortized。And it's the binary search tree plus order one space。

Now how do I do deletions and insertions？Well， I keep a deletion counter and when I've deleted a half that it's in the tree。
 ...
 Deletions might make some trees unbalanced。But this it's particularly simple when I'm using tomb sing。
 ...
 doesn't know what deletions are， doesn't know what tombstones are。

 it just does what it does and the analysis works out。If you are actually doing deletions。
...
So everything all works again。Loar inic worst case searches。

Logrimite amortized insertions or deletions。Binary search tree plus a constant number of additional data。

近。嗯。So。I want to give。Another application of this。Local global rebuilding idea。

It's not about binary search trees， but just to demonstrate that the same idea has multiple applications。

Um and。This is another really fundamental problem， but one that maybe as an undergraduate。
...
But the order is' determined by the value， the order is determined by the user who says。
 ...
 put this immediately after that， so think lines of code。😡。

The user types a new line of code it needs to go between existing lines of code。

 so I need to maintain。A sequence。Subject to the following operations。So insert after。X。

 y means put y into the sequence immediately after x。诶。I need to。ToDelete X。

 so given a pointer directly to x， remove it from the sequence。

And now the interesting thing is scan forward。X comm a K。Scan。Backward。X comma K， this means。

Tell me the next K things after X。In the sequence。Tell me the next K things before x。In the sequence。

So if you want to think about these lines of code， you're going to be inserting and deleting lines of code and then I need to display something on the screen。
 ...
 and that's what will'll fit on the window。睇。So a packed memory array。Is。An array。Of size。Big O of N。
...
And what I want to show you is that I can do。Insertions and deletions in log squared end time amortized。
...
Live in a contiguous span of memory of size order K。So intuitively。
 ...
 what you're doing is you're saying， okay， I have this this array where I want to maintain some stuff。

 but I'm going to leave some gaps。😡，Um in between my items so that whenever I want to insert something。

 you know， immediately after this， there's some room where I can do that insertion。

But then every so often， parts of the array are going to get too crowded。

 and I'm going to need to rebalance them。By spreading the the things out in memory。

 so maybe okay now I insert another item here now I insert another item here oh， it's it's。

There's too much to do， so I'm going to actually take these， I don't know， five items here。

And spread them out。诶y。If you are as old as dirt。You grew up programming in basic。
...
Basic has this wonderful feature that lines of code have to be numbered the way that you write code is you give each line of code a number。
 ...
 you just type line 10 for equals one to n line 20 print hello。Okay， oh wait， line 15。Print eye。

Oh wait， line 12， J equals I over 2， line 15， print J。And so you write the code。

But you write it one line at a time and when you want to insert a new line of code in between two others。

 you had to have left room in the sequence of integers that you're using to label your line。

 so traditional practice was you make all of the line numbers multiples of 10。

And that gives you some freedom to put a few lines of code in there。

 but inevitably you're going to decide， oh， actually I need to insert 15 lines in here and so you're going to have to renumber things。

 but renumbering actually required just typing the line of code again with the new number so you wanted to do it as little as you could hope for。
...
Binary tree。Over the array。Now I'm not actually building a binary tree。

 I'm just saying in your head think about the array is being split into a pair of subars and each of those is being split into pairs of smaller subars and so on。
...
Of the thing you just inserted when you delete。You rebuild。The shout sorry， the deepest。Um。

Not too sparse， what's the right word you know？Well， I'll just say it not too sparse。Ancestor。

And the idea is that I'll have thresholds where I say。

 for a particular interval at different levels of this motionional tree。

 I'll have an upper bound on the density that will trigger a rebuild because it's too dense and I'll have a lower threshold that will trigger a rebuild if the density is too small and rebuild in this case doesn't mean rebuilding a balanced binary tree。
...
Let's first do this。List labeling。So list labeling。

 the idea is that I don't necessarily have to keep my items in an actual physical array。

But I do need to assign every item that's in my sequence， an integer label。😡，Right。

 so I need to handle insert and delete。And then I need to be able to answer label queries so that x is before。

Y in my sequence， if and only if label of x is less than label of y。

So the PAC memory array solves this problem by having the label of the item be its index in the PAC memory array。
...
So one item is going to come before another in the inor traversal of this tree。
 ...
 I branch right that's a one so the interpret as integers X will have a smaller label than y。

And you can kind of interpret the p memory array as a solution to list labeling that uses a scapegoatry。
...
Is just I give you two items and I want to know which one is earlier than the other one in the sequence now list labeling solves that problem by just compare the labels。
...
handle inserts and delets， block squared in。If I use a little bit more space。

Then I can speed up or I allow a little bit more time。 I can speed up the insertion time。

 but this is a lower bound。 So what this means is that the packed memory array。
...
5 expected amortize time。In 2024， it's now。

![](img/7d942806f207614d2fe32ecde3453675_11.png)

呃。So。Big O of log and log cubed log and expected amortize time。This is 2024。
...
This is a case where randomization actually buys you something。
