# UIUC《高级数据结构｜CS 598 JGE Advanced Data Structures — Spring 2025》中英字幕 p13 -10-Mar 11： Persistent data structures.zh_en -BV1kWFGzsEmN_p13-
...
A couple of you have already talked to me about not being completely sure about speaking with the class。
...
So the stretch goal， the direction in which I hope you're going to be pointing your arrows before you shoot them is eventually your project might lead to some publishable result。

But expecting that level of depth in a project where you've only been working on it for， you know。

Even Max one semester。Um that's just not reasonable and I know that and I know that you know that and now you know that I know that you know that。

嗯。So what I'm really expecting from the final project report and the final presentation is much more aligned along the lines of a progress report。

We tried these things， this was our goal。This was our you know lower order sub goal where we figured would'd take some baby steps before we attacked the real problem。

 these are the things we tried， these were the small results that we were able to prove。

 but we're still not entirely sure whether this is going to lead to the final result。

We did consider some special cases， we did consider some simple variants。

These are the next things that we would attack if we wanted to keep working on this。

We don't have an answer to the original question。That。

 to me is an ideal project report at the end of the semester。We tried some stuff。

 we made some progress， we have some ideas where to go next。

 but we still don't know the answer to the problem。Um u。And so this also， I think should guide。

Your picture of what I'm expecting from the project proposal。

Which is to come up with some kind of problem to aim for。

That hopefully has some realistic chance of making some small amount of progress， but again。

 you're not proposing， I'm going to prove that people's NPP in the next six weeks。
...
Or I'm really interested in how different in comparing different approaches to let's say。
 ...
 I want to write a detailed survey of all these data structures， that's the final project。系。

So there's a lot of room here to take advantage of your prior expertise。

 your prior research experience if you have any or your prior internship to the extent that you can without violating NAs。

And matching to the kind of project that you want to do。So again。

 ultimately the goal here is to make progress。Now， this is not a。

 by any means an easy thing that I'm asking， I would expect that。The final project is。
...
I'll double check that after class， but yeah， you should still be able to submit in。Yeah。好庭签定。

You choose them。However you want。I'm not going to assign groups。UmOkay。

But I do want to remind you that project proposals do。Whatever that first Monday in April。
...
The following adjectives in front of the word persistent。So partial is。

 I can only update the most recent version， but I can query any past version。Okay， so。Update。

Most recent。Query。Any past。Version past， I'm using loosely。It includes the most recent version。Full。

 I can either。Query or update。Any version？And confluent， I can do everything that I can do with fold。

 but I can also create new versions by combining old versions。嗯。Come on。Green earring。So u。

These three differ in what history looks like。So for a partially persistent data structure。

History is a sequence。So you can think of。There being a list of versions。

And when I perform an update at one end of that list， I create a new version on the end of that list。

Okay， so I just make the list longer。Full。哦。I'm allowed to branch。So。嗯。History is actually a treaty。

And for confluent。History is actually a dag。So I could do something。Like。This。You know。

 by combining combining versions， get is closest to a confluent data structure。

So today I'm going to pretty much exclusively talk about。Partial persistence。

I'll mention other things in passing and then I'll talk in a bit more detail about full persistence on Thursday but one。

First， one thing that I do want to point out almost immediately。Is that？
...
Then it automatically gives you something confluent， you just。
 ...
 you have a handle on the route of version7 of the data structure and then you wake up three days later and you still have a handle on version 7 of the data structure and looks exactly like version 7 always did because it hasn't changed。
...
Lovely book calledPurely Functional Data Structures by Chris Sokasaki that grew out of his PhD thesis in the late 1990s。
 ...
 even if you have no interest in doing both of those at the same time。系。

Because it's just really cool。Okay。嗯。So。啊。A couple of warnings though， about。Inftant data structures。

In terms of efficiency。So ideally what I would like。Is so the goal。

Is if I want to do a query into a persistent data structure。What I like is as much as possible。

Query time to be the same as the query time in that particular version of the ephemeral data structure that I' modified。
...
So I've got some say binary search tree over all possible versions。

 I know the version number I just don't have the direct handle to it that extra step adds a little bit of overhead。
...
Ideally should be proportional to the number of times through the history of the data structure that I needed to write into memory。

So you could， as a sort of crude upper bound aim for any constant amount of time I spend on updates can generate a constant amount of space。
...
Or if you want to say differently， the number of times some piece of the data structure need changed。
...
That no longer works。And so the reason， you this is sort of incompatible。With。U。Ephemeral。

Amortize time。And the reason is pretty simple， let's take Sp trees as a particularly bad example。

 so spplay trees guarantee to have log n time per access to any node that's a search insert search or deletion that's amortized time but spplay trees have no guarantees at all on the worstcase time to do a search。

And in fact， it's possible to engineer a sequence of ss that makes the depth of the tree linear。

So let's imagine an adversary who creates a linear depth displayplay tree。

And let's call that version7 of the Sp tree， and now the adversary goes， okay， query， query， query。

 query， query， hey， go back to version7 and query that really deep node。Okay， lah， la， la， la，la， he。

 go back to the Dorsion 7 and change that really deep node from a7 to a6。Okay。

 go back to version7 and query that really deep note。
...
Okay， so the user。Of the persistent structure。Can repeat。Bad。Querries。Yin。😔，The same。Version。

Of the ephemeral data structure。Okay， so。When I talk， for example about。

Multiple short shortest paths。I said， oh， hey here's this we're going to be using these dynamic forest data structures。
...
We don't know what distances the user is going to want to ask about。
 ...
 but it's fine we just make that data structure persistent。

And then later if data if the user wants to know， hey。

 what's the shortest path distance from this node on the boundary to this other node。

 it goes back to the version of the shortest path tree where this node on the boundary was the root and it does a query for the distance at that other interior node。

 that's great except when we described all of the dynamic forest data structures。For simplicity。

 I just built them out of Sp treess。So。In order to make that data structure persistent。

And still efficient in that persistent setting。I can't use sigh trees。
...
系。Um， so then I have to go back and go， okay， there are weighted versions of red black trees。
 ...
 there are weighted versions of AVL trees， there are weighted versions of Be trees that I can use here。

But I actually have to do that in order for persistence to work。

 no matter how I implement persistence。Because I could always just get unlucky and go back to that old defemeral version that had some node at linear depth。

 and that's the one I want to query over and over again。嗯。There's a second problem with s trees。

Which is this whole architecture is sort of built around the idea that the ephemeral data structures in past versions are read only。

But Sp trees， when you ever you query them， you are also doing an update。

So that's fine if you know you want to do。Full persistence because then every query into the past is branching off a new version。
...
As lovely as play trees are for other things。In terms of persistence。Spplayre suck。He。
 ...
 we got to use something else。诶。And I might mention in passing when it comes to that。
...
And that's the idea of just a。Resistent stack。Okay。

 so a stack is a data structure that maintains a sequence of things。

You can add something to the beginning of that sequence， that's called the push。

 you can remove something from the beginning of that sequence。

 that's called the pop you can ask hey what's at the beginning of the sequence that's a top so the kind of updates you can do or give me a new stack。
...
And you can ask whether the stack is empty。Okay。So there's a really， really easy。

 really simple way of implementing stacks。Persistently。And ultimately。

 this is your first introduction to Lisp if you've never seen LispP before。So。

LiISspP is designed around the idea of manipulating lists。

Sequence finite sequences of thingss usually surrounded by parentheses， but under the hood。

These lists are actually represented by binary trees。Now they're really skewed binary trees。

WisP gene you know represents the list you know ABC by a node that has an A on the left。
...
Yeah。下公。thatOkay， so for now， let me assume that you actually have a pointer directly to the head of the version that you want to query。

I mean， later what you would do is if you're doing at least partial persistence。

You would just keep a record， an array a binary your search tree of all versions over time where where I could use。

Do binary search， for example， given the timetamp that I want， it will return。

 it will I can find to the pointer to the head of the data structure at that time。比较上。吹排咧。Yeah。

 so one of the fundamental assumptions I need to make in order for this to work。
...
So that's what I'm referring to as the head of the data structure。
 ...
 the more general term I'd guess is access pointer。But here。

 ultimately what I'm going to do is I'm going to represent my stack as a list。系。And Lsp defines。
...
系。嗯。Pop is。Hey， just give me a pointer to the right child。Top is， hey。

 just give me a copy of the left child。And is empty is。You know， nil S。If I want to write these。

 not in list。啊。I can say this is S sta left。This is return S dot right。This is， you know。

 S equals null。And push is creating a。New node， why is it not mirroring？Interesting。嗯。😊。
...
Right， so push is it creates a new node with an explicit left pointer to X and an explicit right pointer as。
...
And tested empty and so on Okay so this is sort of one of the ways that you know。

 doing things in a functional programming language actually helps when you define a new data structure。

 you create objects， but you never actually changed the objects themselves。
...
Going to have a bunch of pointers flying around。 So if I decided to take this stack and pop off the a。
 ...
 then I would get a pointer to to this note here。 And then if I decided to push on an X。

 I'd get a new pointer。That。Points to。An X the left child and its right child would point to the nerd。

And so you end up with this weird conglomeration of ultimately， it'll still be a forest。UmUh。

 but it'll be just a forest where different versions will point to different nodes in the forest and go。

 well， from here down，um I just have a regular binaryary tree that represents a list。嗯。O。So。

That's sort of the basic idea。Of what persistence is， any questions so far？Okay。So。嗯。Let me。

Talk about one method for doing this。And I'm talking about these methods for making data structures persistent in a fairly generic way。
...
The the data structure consists of。A rooted forest。So it's a pointer based。Structure。

I have a constant number of。Root pointers。And for every。Noode。Has a unique。Pass。From。A unique。Great。
...
Start at that address and walk my way down to the note。Okay， so this is not all that restrictive。
 ...
 larger tree data structures have this property， forest data structures have this property as long as the number of trees never exceeds the constant。
...
You will say a binary tree， and suppose I want to make some sort of modification at this node my binary tree。
...
one example to keep in mind is the tournament trees that we used as one of the ways of solving the minimum range query problem at the beginning of the semester。

 so you imagine a perfectly balanced tree that has values at the root and then every internal node stores the minimum of the values of its children and then if I want to do a minimum range query I can do it by accessing logarithic number of these nodes。
...
But in the persistent data structure， I have to propagate the copying all the way to the root。
 ...
 even if the only data that's changing is in that one node。系。

So now this would be like this is version one and after that update， this is version two。

Buts being now is parents， so the version three has to point version to them right？Right。

 so if I didn't update the parent， then in particular， I wouldn't update the route。
...
Now this is one way to do that is just to say， okay， I' mixed every time I have an update。
 ...
 I'm going to create a new route with that the version pointer points to and I could just chase the pointers normally from there。

 this gives you the path copyping thing， this is not the only thing that one could do。

But we'll talk about other methods。In a few minutes。系。So。啊。Querries。Is to sort of just。Like。

The ephemeral structure。So if I now want to do a search in version two of this binary tree。

I just treat version that pointer that node that version  two is pointing to as the root。

 and I do exactly what I would normally do in the ephemeral data structure。
...
If this is a tournament tree， the values at the nodes are still the minimum of the values that their children。

Do I just。Only see the new version effectively okay。Updates。Well。

The amount of time to do the updates is for the update to one node。Is the length of that path。So u。

This is。嗯。Let's say。啊。Path length。Per node。Update。And this is both。Time and additional space。喂。

So exactly what that is it's going to be data structure dependent， but for example。

 if this is a balanced binary search tree， that pathlink is always going to be lo n and if applied naively it's per node update now if you happen to know that during an update。

 the only nodes that are going to change all happen to lie on that one path and you could just copy that one path。

If you happen to know that there's some top subte that contains all of these changed nodes。

 you could just copy that top subte， but sort of naively， if I think of it as well。

 I don't want to think about how the data structure is organized。

 just whenever you write into that node， I need to create a path a copy of the path leading to that node。
...
This is for off the shelf binary search tree。嗯。For most balanced binary circuit trees。
...
ok。So one of the things that one might need to do。If figure out a way of taking。
...
A more interesting data structure， a binary search tree that has parent pointers。
 ...
 by just being very careful with how you manipulate stack frames。

 so you recurse your way down and you pop your way back up and essentially you're storing your own local copy of that entire path going down。
...
And this is going to be subject to operations that are local to the finger。You can。
 ...
 but。啊。It also allows you， for example， to move the finger and that will change the zipper data structure that represents it and so using this zipper data structure it's a little bit easier to implement things like walking down up path in a binary tree and then walking back up。

So the idea。Is let's。Take， I'll just pick a。Any binary tree here？A， E， C， D， E， F。GHI。

 and let's say that I my finger is here。The intuition。Is that I reverse？The root， the path。From。

The root。To the finger。So again， thinking about this in terms of pads。

Now all of these edges I'm imagining are in the abstraction pointers going down。
...
This is exactly the same game that we're playing here。
 ...
 so in the representing the zipper data structure that have a node F which has left child H and right child I。

 but has parent child。C。Parent the node C has right child G， and it also has parent child A。

Parent child A also has a。Right left child。第。B。In addition。
...
Each of these nodes on the path that whose parent is below them。Records a bit that says， hey。
 ...
 A isn't a child of anything so that bit doesn't make any sense。Okay。

 so the tree based representation of3+ pointer。Is this slightly inverted tree where now the root might have three children？

All every other node in the tree might have w only f2， but of three possible species。
...
And if it has a parent child， it also has a bit to say。
 ...
 I am the left in the tree that I'm representing， I'm the left child of my parent。
...
The right child of C will be G and the parent child of C will be A。So whenever I moved the finger。
...
I only need to swap the order of two nodes， which means I only need to update I don't know。
 ...
 red black trees using the zipper。Then and in fact。

 I can do this in a way there are Python packages that you can say， oh。

 you've got a nice little pointer base tree， just put a wrapper around it to represent it as a zipper and in fact。
...
Functionally。As functional data structures Okay， so the user just thinks this is the data structure。

So the user's queries and updates are all in terms of this data structure。

Those update subroutines involve moving a finger around。And so internally。
...
But it's all done sort of locally， and even though it's locally path copying。
 ...
 all of the internal paths that I'm copying just have constantly。Okay。Great。Um。So among other things。
...
The zipper is the data structure。The user thinks this binary on the left is the data structure。😡。
...
找到你们试一下。You write a date you say oh， I have an AVL tree here's the here's the algorithm for inserting something into an AVL tree。

 let me look at the code oh， there's a local variable。Pointing to a note。That's my finger。

You choose the finger by basically characterizing how your ephemeral updates work。In terms of。
...
It's not like I'm designing a new update algorithm。
 ...
 I'm trying to exploit existing update algorithms in this functional way。both。In in structure。

Right nowOkay， so over here on the left， if I wanted to do a rotation at F。 Okay。
...
Figure that right do left。Thought right goes to figure out right。
...
You can write the rotation。Carefully by doing a bunch of local。Pooner Jason。高于A是简单。
...
You don't want to say anything else about half copyping， no， I don't think so。

I want to talk about the other。Sort of baseline implementation of persistence。

Something called fat nodes。嗯。This means something else in。Systems land。This just means big。

So here the only thing that I am assuming about my data structure。诶。Consists of。Order one space。

Uh he says。Which I'll call nodes。嗯。But this doesn't necessarily have to be a pointer based data structure。
...
系。And then the idea is really simple each node。Stores。It's complete。Loal。History。Okay， so。Well again。

 I have a balanced binary tree。I just watch this node V and the code says change V dot left。

 it's like， okay， new version， V dot left。Okay， change V dot right， Okay， new version， V dot right。

 change these parent， Okay， V parent new version， changed the search key the Okay。

 great new V dot key， there's a new version。Every time you write some new information into the node。
...
A you use a constant amount more space in your local history。
...
I define time to be the number of updates。And so when I want to query a version。
 ...
 I pass into the timestamp。Says， I want to know what to do with this node in version 17 to the data structure。
...
We do know that the timestampance。Or by definition， integers between one and capital n。
...
To do those predecessor query in log log end time。All right， so it's not log n， it's log login。😡。
...
And one of two things must be true， either there is a timestamp that has that same in my history。
...
So it's essentially a divide and conquer algorithm over the binary representation of the timetamp。
 ...
 so the time to do a query is going to be whatever the earial query time is。Times this overhead。
