...
现在我知道我不需要再往前走了，因为这都是两个属性关系，对呀，所以我不需要再分解了，我受够了，我知道两个属性关系是nbcnf，好的，那么BCNF给了我们什么，所以BCNF所做的是消除某些类型的冗余。所以它基本上消除了冗余的类型，哪里啊，本质上是由功能依赖关系所暗示的，对所以如果你有，嗯a意味着b，这意味着如果你把A和B放在一起，与关系中的其他属性一起，然后对其他每个属性重复B值，因此。BCNF摆脱了这些类型的冗余，所以对于冗余的类型，BCF并没有完全消除，比如说，那些被称为多值冗余的，我鼓励你看看课本，所以还有其他类型的货币BCNF不处理，事实上，还有其他类型的依赖关系。您需要对这些随机状态进行推理，这就是所谓的多值依赖关系，又好了，看看教科书，如果你有兴趣，所以BCNF也有一个不错的属性，其中避免任何信息丢失，对呀，所以呃，基本上是无损分解，所以你可以重建最初的关系。

所以让我们说，我把关系r分解成，第一和第二，分别包含a b和b c，我将如何用r 1和r 2重构，任何想法我都会做一个自然的连接，对，我会在B上做一个自然连接，这正是这两种关系重叠的原因。这样我就可以对共同的属性进行联接，好的，所以这就是为什么我有这个属性，这些共同的属性，在两种关系中都重复着，这样我就可以做一个自然的连接，然后重构原来的关系，好的，所以这一点的证据可以在教科书中找到。但总的来说，我想让你记住的财产，这是一个无损分解算法，好的，所以现在一个问题来了，我们能作弊吗，所以我们看到了两个属性关系r nbcnf，所以我们为什么不把这个任何关系r，然后把它分成两个属性关系。同时确保它们之间有一些重叠，这样我们就可以做这个连接，然后就做对了，我们为什么要费心寻找BCNF的违规行为，为什么我们要做这种函数依赖的废话，为什么我们不能像分解为两个属性关系，并被完成。

事实证明这种方法会导致信息丢失，所以如果你不小心你是如何进行这种分解的，通过识别违反的功能依赖关系并使用它们进行分解，您最终会出现丢失信息的情况，所以让我们举一个例子，假设我用这个关系式r。它具有属性a b和c，然后我随便把它分解成两个关系，b和b c好的，所以我取了关系R，在关系r 1和r 2处分解为2 1有a b，另一个有PC，那么我如何进行分解，基本上我，它本质上是一个投影权。所以我有这个关系r和我有这个关系，包含a和b的r，这个关系r 2包含b和c，如果我在r 1和r 2之间做一个自然连接，我基本上以R的超集结束，我得到了一些在原始关系中不存在的假元组，所以总的来说。如果我只是说嘿，让我把这组属性，然后就像，把它分解成两个不太管用的属性关系，你实际上必须遵循这个严格的程序，这是由BCNF概述的，嗯，要正确地做到这一点，这样你就不会得到假的元组，使其无损，好的。

所以这必须是一个无损的过程，所以这是一个总结，嗯，所以总的来说，bcnf，呃，基本上，确保每个字段包含不能通过功能依赖关系推断的数据，嗯，所以没有冗余，所以这允许。或者至少没有来自功能依赖的某些类型的冗余，所以为模式输入BCNF实际上是一个很好的启发式，以确保您的架构是好的，所以如果你不在BCNF，你基本上试着把它分解成BCNF关系，现在BCNF的一个缺点是。并不是所有的依赖关系都得到了保留，好的，所以有些依赖关系，一些功能依赖实际上可能会在关系中分裂为什么这是一个问题，所以这是一个问题，因为执行这些功能依赖变得很有挑战性，对吧，因此。实际上很难强制执行这些功能依赖关系，如果函数依赖关系中引用的属性在关系中拆分，所以很难执行，现在这是阿尔文最喜欢的原则的另一种情况，瘦员工，嗯，所以基本上你知道，这里没有免费的午餐，对呀，因此。

如果您想保留依赖项，那么你最终会有冗余，如果你不想裁员，您将无法保留所有依赖项，好的，所以如果你想理解这些权衡，我鼓励你看一下教科书，好的，看起来任何，呃，关于这东西的问题，所以尼古拉斯问。我想只有运行BCNF分解才有好处，如果您有大量的元组样本，对呀，否则，您最终会得到一堆不太有用的推断函数依赖项，所以功能依赖关系是正确的，从某种意义上说，这已经为您的应用程序修复了，嗯。你基本上想运行BCNF之前，呃，添加数据，嗯，所以你基本上提前运行它，预计你最终可能会有一个相当大的关系，您可能必须处理申请级别的冗余，如果你不小心，对呀，所以你基本上是提前处理的。你计划在哪里把它分解为良好的关系，然后将其用于应用程序，BC和F如何影响性能，好问题对吧，所以呃，就像阿尔文提到的，加入是不免费的，对呀，所以BCNF和分解的一个缺点，你基本上必须做这些自然的连接。

因此，当您试图为查询提供服务时，这将增加性能成本，所以不做正常化可能还有其他很好的理由，也就是，如果您总是查询关系的所有属性，同时，您可能更喜欢将所有这些属性放在一起，即使现在以裁员为代价，如果它来了。如果你有冗余，那么您必须以某种方式处理应用程序中的冗余，您必须确保相同属性值的所有副本，或彼此同步，所以这是你必须支付的罚款，如果你想要不做关节的性能好处，所以这是空间和性能的权衡，是呀。这是一个很好的思考方式，所以我把杰瑞的观点称为，但同时不仅仅是空间，对呀，这也是必须处理冗余的开销，事实上，你可能有，呃，异常现象悄然而至，如果你不小心，任何其他问题，好的，你看嗯，让我进入下一个话题。
...
阿尔文，我应该，呃，我们应该，我们现在该休息一下吗？还是我们应该在十五分钟内做完？不管怎样，我是说，你们觉得，十五分钟，好的，所有的权利，并行查询处理，那么嗯，所以让我给你一点，让我从。应该完全改变话题，对呀，所以我们要从数据库设计回到数据库内部，对呀，所以我们是在上一节课上讲到的，如果您试图计划如何为应用程序构建数据库，你会怎么做，现在我们回到数据库系统开发人员。思考如何进行查询处理和优化，当你有，嗯，很多很多，呃，比我们过去考虑的资源多得多的资源，所以让我们从谈论一点历史开始，对呀，所以在20世纪70年代，传统的查询处理概念涉及，嗯，基本上是一台机器，对。我们二十世纪七十年代带来了，元组集合上的声明性查询的概念，是的它成功地引入了物理数据独立性的概念，逻辑数据独立性，以及数据库系统可以透明地优化查询的事实，来支持，高效率，最终用户访问权限。

因此最终用户提供SQL，系统为您优化，但这一切都集中在一台机器上，现在在二十世纪八十年代，有一种意识，你可能想，通过利用并行性使事情变得更快，所以有一个，呃。20世纪80年代并行关系数据库数据库系统的出现，这基本上啊，了解到可以瘫痪这些声明性查询，这些SQL查询，从而让他们跑得更快，所以，这些并行关系数据库系统使用的典型体系结构是，使用多台商品机的权利。经常像几十台商品机，并用它们来瘫痪SQL查询的处理，即使从商业角度来看，也有一些初创企业和公司最终获得了相当成功的并行数据库系统实现，然后在两千年，平行化的想法在呃的背景下死灰复燃。像MapReduce和Spark这样的系统，所谓的大数据时代，就在2000年中期，所以有人会说这个大数据时代一直持续到今天，对呀，所以呃，向拥有少量商品机器的转变，到可能成千上万的机器和更远的地方。

并且能够在它们上支持并行查询处理，那么为什么平行是个好主意呢，对呀，所以让我们从扫描一个关系开始，假设它现在有100TB，如果你假设你的呃，您的磁盘可以读取数据是，假设每秒半千兆字节。现在大概需要两天的时间，两天多一点扫描100TB，所以加布提供了更多，呃，周围的上下文，嗯，在幻灯片周围，所以我鼓励你看看他的背景，那太好了，好的，所以呃，所以如果你试图用传统的，呃磁盘。这大约需要两天时间，嗯，所以这就像用一根吸管喝水，对呀，所以这是一个类比，用一根吸管喝水，大概需要两天的时间，如果您有多台机器，就像一下子喝了很多吸管，对呀，所以它基本上允许你并行地做事情，而你。假设你有一百台机器在你的酒桶里从这些机器里喝水，每台机器的速率相同，所以每秒半千兆字节，嗯，这将减少到3-3分钟，对呀，所以这是一个相当大的速度，对于这个100TB的数据集，从两天增加到三三分钟。

我们不是还受到嘴巴空间的限制吗？这很公平，至少这就是稻草的类比失败的地方，所以这很公平，所以你可以看到这个人承受着多大的压力，好的，所以这里的高级想法是并行化解决了一个大问题，然后把它分解成很多小问题。希望你能让他们尽可能独立，所以每个单独的问题都是尽可能独立的，这样他们就可以按照自己的节奏前进，不需要互相协调，在这些小问题上任何必要的协调都会导致，所以你不想让他们做很多协调，所以你想让他们独立，你。他们每个人都以自己的速度前进，谢天谢地，大多数关系运算符实现都可以独立，这样它，它是平行发生的，没有太多的协调，所以这里有两个指标，我们希望为并行查询处理而努力，好的首先回想一下吞吐量。这是我们感兴趣的指标，是我们可以支持的事务数，呃每秒，我想我以前把它定义为每秒查询，但现在你知道了，交易的概念，基本上每秒的事务，因此，我们可能对并行查询处理上下文感兴趣的一个度量，就是加速的概念。

所以加速的概念是说如果你增加硬件资源，假设您正在使用的机器数量，但你修复了工作量，吞吐量增加多少，所以这里，你想要的基本上是一条曲线，随着硬件的增加而线性增加，好的，所以通常，尽管你可能有比，呃，理想。另一个指标是放大，所以这基本上是在捕捉场景，在这里，您既增加了硬件，也增加了工作负载，所以你同时增加了两者，所以在这种情况下，您想要的是吞吐量保持不变，因为您增加了数据大小，再一次的平行。即使这是理想的，你想呆在平地上，通常情况下，随着你的增加，这可能会下降，所以这是我们关心的两个指标，加速和扩大规模，在任何数据处理系统中，所以要成为关系数据库，或者有两种类型的并行性，我们可能会感兴趣。所以并行的第一步就是所谓的管道并行，根据管道深度缩放，好的，以便根据运算符的数量进行缩放，管道中的程序数，所以这里，假设我有这三个按顺序操作的运算符，或者您可以将它们视为关系运算符。

但它不需要是关系运算符，可以是一般的数据流，每一个都是按顺序发生的，嗯，所以你基本上从对元组应用一些函数开始，嗯，我们假设x 3，然后应用g，下一个程序应用于其结果，然后将h应用于其结果。对但这里的神奇之处在于这三个程序可以应用于，同时，对呀，因此，每个程序并行地对不同的数据项执行不同的操作，所以这三种程序或运算符是在元组上操作的，在项目上并行操作，一次又一次。这个管道并行的想法不仅仅是特定于呃，关系查询计划，我们稍后会更详细地讨论，但它也普遍适用于任何数据流范式，比如说，机器学习操作符，而不是关系运算符，所以另一种类型的并行性是分区并行性，所以分区并行性。与管道并行性不同，它只扩展到管道深度，分区并行性可扩展到数据量，所以基本上你要把这个程序分区，使每个程序并行地对不同的数据项应用相同的操作，所以嗯，对于函数f，函数f可以并行地应用于不同的x，好的。

这就是分区并行性，基本上就是，呃，扩展到数据量，管道平行度扩展到管道，当然，您可以将管道并行与分区并行结合起来，我们稍后将在关系查询处理的上下文中讨论这一点，所以即使我描述了这两种类型的平行。中的管道并行性和分区并行性，在一般数据流范式的背景下，它在数据库上下文中尤其重要，首先注意你有很多数据，并且有可并行化的运算符，对呀，所以你基本上是在尝试对数据集进行批处理运算符，一次元组集关系。你有算法，实现，这些运算符的物理实现，本质上是在做某种分而治之的方法，随后我们将讨论这些算法的几个例子，另外，您有一个自然的地方，流水线在迭代器模型中出现，对呀，所以你有这些个人，呃。作为i交易者的运算符实现，所以你有这些单独的运算符处理元组，呃，同时，另一个好处是，您正在做的所有这些并行化都可以在后台完成，没有这个，用户甚至意识到并行化，对呀，因此，您可以更改任务和硬件的并行策略。

所有这些都不改变用户实际试图做的事情，因此SQL查询不需要更改，这基本上是物理数据独立的权利，所以SQL查询不需要更改，嗯，正在改变的是查询处理方法和您正在使用的硬件，而不是用户看到的。我们将要讨论的是，从80年代的并行数据库系统工作中产生的一些见解，嗯，很多都被重新想象或重新学习了，在2000年大数据系统的背景下重新审视，好的，所以在我继续问到目前为止的任何问题之前。也许我要在这里休息一下，为什么你们在想问题，我想把这个话题带回来，嗯，为什么我们现在要谈论这个，所以如果你看看课程，呃总体来说，我们实际上已经教会了你们所有关于，像单节点数据库，信不信由你。所以我们决定这样组织主题不是一件随机的事情，为什么我们要从设计理论转向并行数据库，基本上是因为，到目前为止，你们都是单节点查询处理的专家，对呀，所以你知道从如何在磁盘上存储数据的一切。

如何运行查询运算符，即使是现在，当您知道如何正确地设计模式时，这样它们就可以方便地有效地完成这些查询操作，所以在这一点上你应该会觉得出去很舒服，就像你在编写运行在单个节点数据库上的应用程序一样。我想现在你们都应该是专家了，所以我们现在转向并行处理的原因，是因为那基本上是下一个前沿，对呀，所以我们基本上覆盖了单个节点的所有内容，所以我们现在需要覆盖的剩下的东西基本上是发生的事情，当我们试图运行。呃，跨多台机器的数据库，以及当我们试图在多台机器上运行事务时，事务处理会发生什么，所以信不信由你，这基本上是我们本学期剩下的两个主题，除了说没有续集，这本身就像是一个话题。但我只想向你们解释为什么我们要组织课程结构，呃，这边，这不是偶然的，基本上也是因为我们想折磨你，否，我开玩笑的，是啊，是啊，所以当我们平行讨论时，你们基本上会看到，呃，这在本主题中。

也在下一个交易主题中，同样的东西会再次出现，对呀，基本上很好，你怎么，我们怎么数，iOS右，当我们试图并行运行事物时，我们最喜欢的区域会发生什么，对呀，和所有其他像你知道的，当我们试图运行，是啊，是啊。我知道，对呀，会很头疼的，但就像，你知道，嗯，这基本上就是我们所看到的，当我们试图，当我们试图并行运行事物时，好的，回到你的，太厉害了，有什么问题吗？我认为这是有价值的背景，谢谢，我会的，好的。否则我会分享屏幕，让我们看看，先暂停录音，是啊，是啊，让我们暂停录音，好的，还有什么问题吗，呃，好的，所有的权利，嗯，所以在如何使用并行查询处理技术方面，嗯在历史上，在许多场景中。已经探索了这些并行查询处理技术，这是人们研究并行化的三个主要架构，一种方法是使用共享内存和共享磁盘，它基本上被称为共享内存，嗯，所以这基本上类似于现代计算机，对，所以如果你用的是笔记本电脑。
...
然后阿尔文将用它来推广到各种运算符实现，好吧，所以他会谈论，呃，排序和排序，合并和连接算法，所有这些所以请记住我们有一个散列方法涉及多个阶段，嗯，所以在这种情况下，哈希方法实际上非常简单。你基本上有一个首字母，呃，在不同的哈希函数上哈希的洗牌阶段，其后，每台机器独立于其他机器运行，好的，第一阶段的前期基本上是将数据流到网络上，以及基于某种哈希函数的数据洗牌，h n，确保。所有对应于给定哈希值的元组到达给定节点，一次每一个的所有元组，呃，每个哈希值都到达一个特定的节点，那么这些节点中的每一个都可以相互并行地进行，这个的好处是，当你读这个数据的时候。当您在这些节点上读取这些元组时，他们可以开始把这些元组流出来，这样他们就可以把这些元组流到网络上，尽你所能，嗯，你基本上只是嗯，应用哈希函数h n来确定，呃，把它送到哪台机器上，所以接收器。

这些机器得到这些元组，然后可以进行哈希的下一阶段，就像在单节点哈希设置中一样，好的，所以当你得到这些元组时，他们可以申请，呃，他们的本地哈希函数h p和h r和以前一样，在网络上洗牌数据的过程。这样您就可以应用单节点方法，这是相当快的，并且具有近乎完美的速度和规模，因为你基本上是用分区HP流式地完成所有这些过程，这是传统单节点哈希过程的第一步，与在网络中洗牌数据的边缘和过程一起发生。所以你真的不需要等待来应用HP，但是，您确实需要等待，直到您决定进行重新分区，所以你必须等到第二阶段开始，嗯，你需要考虑到歪斜，潜在的，呃，是什么导致了散兵游勇，所以基本上可能会有病例。那里有一些机器还没有接收完所有的数据，还没有准备好进入下一阶段，所以你要等那些，呃，然后才能开始下一阶段，所以哈希实际上是非常可并行的，你基本上把数据洗牌到机器上，然后每台机器自己进行。
