# 【计算机体系结构】普林斯顿—中英字幕 p101 100_05_network-performance -BV1ii421D7WR_p101-

Okay， so now we start talking about performance。Of our networks。And there's two。

Main thoughts I want to get across here or two really good ways to measure our networks。
...
Yin。Seconds， so the unit on this is seconds。 The unit on this is something like bits per second or bytes per second。
...
So it can actually help with Laty。It can also help infency because it can help reduce your congestion on your network。
...
But by having more bandwidth， it'll， you can effectively reduce the load of your network。
 ...
 and that will decrease your。It'll decrease the load of the network and decrease the probability that you're actually going to have two different messages contending for a same link in the network。
...
Latency can actually affect our bandwidth， which is interesting。
 ...
 it's not going to make our links wider or our clock speed of our links faster。
...
And if you can't cover that laency。If the lanes here to get longer。
...
Now， if you are good hi， this latency by doing other work， that may not happen。
...
So a good example of this is in TCP IP networks， something like our ethernet networks。
 ...
 there's actually a round trip of flow control between the two endpoints。
...
More traffic in flight to cover the round trip latency。
 ...
 and if you increase the latency，The bandwidth will effectively go down if you do not allow for more traffic in flight you wait before you can hear a flow control response。
...
4 node Oomega network here with。Two inputs， two output。Routers。

Each of these circles here represents input nodes， and these are the output nodes and they basically wrap around。

 they're the same。Sort of thing。We have little slashes here。
...
So what this means is you're transmitting some long piece of data， and it gets。Sent as smaller。Fits。
 ...
 if you will。 So we're setting on say， a 32 B。Word。

 and it gets serialized into four8 bit chunks across our network。

 across the links because the links of the network are only for or excuse me 8 bits wide， we'll say。
...
Across one of these routers here， and then we get to the destination。And if we look at this in time。
 ...
 it's pipelined。 We can have multiple of these things going on at the same time。
...
If you were to increase the bandwidth here， the serialization latency would go down。嗯。Then we have。
...
So you can see that it's the summation of all of these different latencies is our latency。
 ...
 but what's interesting to see is that there is no decsialization latency here。

So that's the one that's missing。 And it's because we've overlapped that because as pipeline。

 we're counting that。In the serialization Laency。Questions about that so far？Okay。

 so now let's take a look at。Our message lay in C in go into a little more detail here。

If you look at our overall latency， which we'll denote as T。It's the latency for the head。

To get to the receiver。So that's all of this stuff here。Plus。The serialization latency。Now。Tea head。

Has RTC and R TR and a number of hops， but it also has something here。This is a contention。

Which we haven't shown。 So in this network here， there was no contention。

 this is an unloaded network。There was not multiple nodes or multiple messages trying to use one outbound link or use any one given link in this design。
...
You can make the routers faster。So you can either increase the clock frequency of the routers。
...
The laency of the network goes up because you start to see more congestion in the network。

So the probability。That any two points are contended for。

Will go up as you get closer to the maximum achievable bandwidth in the network。Now。

 there are some networks that people build where this is the graph does not really look like this。
...
😡，Because as you increase your load on the network， everyone can send to everyone else。
 ...
 so there's not going to be a congestion in the network。

And I have a few lines here that sort of show。Interesting。Things that sort of hack down at this。

 So in a perfect world， you'd have your zero load latency。

 So this is the latency of the unloaded network。 And as you increase the bandwidth。

It wouldn't change。On our unloaded network。And with so。And and if you had no。

 no congestion in the network。But we but that's not usually what you see on real world networks。

Coup other things sort of。Also。Decrease or increase the latency and decrease the bandwidth of a network。

Usually you have some。Roouting delay。That gets introduced into the network。

 and that's going to basically push us away from higher bandwidth and lower latency。

 So you want to be farther down on this plot because that's the lower latency。And also。
...
But just wanted to give you guys the idea here that。For any real world network。

 it usually looks something like this as you get closer and closer to using the whole network。

Using all the bits available by the network， the laency starts to shoot asymptically through the roof。
